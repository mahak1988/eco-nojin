...
# -*- coding: utf-8 -*-
"""
🌡️ افزودن نقشه‌های هواشناسی و حرارتی
- کنترل زوم سمت چپ
- لایه‌های هواشناسی (دما، بارش، باد)
- نقشه حرارتی پیشرفته
- شاخص‌های هواشناسی
"""
import sys
...
# ========== 1. به‌روزرسانی ProductionGIS ==========
def update_production_gis():
    print("\n🗺️ به‌روزرسانی ProductionGIS.tsx...")
    
    content = '''"use client";

import { useState, useRef, useCallback, useEffect } from "react";
import { 
  MapContainer, TileLayer, Circle, Popup, Polyline, Polygon, Marker, 
  ScaleControl, useMapEvents, useMap, ZoomControl, LayersControl, HeatmapLayer
} from "react-leaflet";
import type { LeafletMouseEvent, Map as LeafletMap } from "leaflet";
import "leaflet.heat";
import { 
  MapPin, Ruler, Square, Layers, Search, Download, Eye, EyeOff, 
  Loader2, Save, Trash2, Navigation, Target, X, Check, Plus, 
  FileText, Globe, Printer, Crosshair, Satellite, Mountain, Droplets,
  Thermometer, Wind, CloudRain, Cloud, Flame, Gauge
} from "lucide-react";
...
// ============ لایه‌های هواشناسی (OpenWeatherMap) ============
const WEATHER_LAYERS = {
  temperature: {
    ...
    opacity: 0.7,
    color: "#ef4444",
    description: "دمای سطح (°C)",
    apiKey: "YOUR_API_KEY", // نیاز به API Key دارد
  },
  precipitation: {
    name: "🌧️ بارش",
  ...
  clouds: {
    ...
    opacity: 0.7,
    color: "#94a3b8",
    description: "پوشش ابری (%)",
  },
  humidity: {
    name: "💧 رطوبت",
    url: "https://tile.openweathermap.org/map/humidity_new/{z}/{x}/{y}.png",
    opacity: 0.7,
    color: "#06b6d4",
    description: "رطوبت نسبی (%)",
  },
  pressure: {
    name: "📊 فشار",
    ...
    opacity: 0.7,
    color: "#8b5cf6",
    description: "فشار هوا (hPa)",
  },
};

// ============ داده‌های نمونه برای نقشه حرارتی ============
const SAMPLE_HEAT_DATA = [
  { lat: 36.3, lng: 59.6, intensity: 0.8 },
  { lat: 33.5, lng: 54.5, intensity: 0.3 },
  { lat: 31.5, lng: 51.5, intensity: 0.9 },
  { lat: 28.5, lng: 60.5, intensity: 0.2 },
  { lat: 38.0, lng: 46.5, intensity: 0.7 },
  { lat: 29.6, lng: 52.5, intensity: 0.6 },
  { lat: 35.7, lng: 51.4, intensity: 0.85 },
  { lat: 32.6, lng: 51.7, intensity: 0.75 },
  { lat: 36.8, lng: 50.6, intensity: 0.65 },
  { lat: 34.3, lng: 47.1, intensity: 0.55 },
];
...
${locations.map(l => `<tr>
  ...
  <td>${l.coordinates[0].toFixed(5)}, ${l.coordinates[1].toFixed(5)}</td>
</tr>`).join("")}
</table>
</body></html>`;
  ...
  return (
    <div className="relative h-full">
      ...
      >
        ...
        <TileLayer
          key={baseLayer}
          url={baseLayer === "nasa_viirs" ? getNasaUrl() : BASE_LAYERS[baseLayer].url}
        ...
        {/* Weather Layers */}
        {Object.entries(weatherLayers).map(([key, visible]) => {
          if (!visible) return null;
          const layer = WEATHER_LAYERS[key as keyof typeof WEATHER_LAYERS];
          return <TileLayer key={`weather-${key}`} url={layer.url} opacity={layer.opacity} />;
        })}

        {/* Heatmap Layer */}
        {showHeatmap && (
          <HeatmapLayer
            data={SAMPLE_HEAT_DATA.map(point => [point.lat, point.lng, point.intensity])}
            radius={25}
            blur={15}
            maxZoom={10}
            gradient={{
      ...
      <div className="absolute top-4 left-4 right-4 z-[1000] flex flex-col md:flex-row gap-2">
        <div className="bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-2 flex items-center gap-2 flex-1 max-w-xl">
          ...
          <button onClick={handleSearch} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs rounded-lg font-bold">
            جستجو
          </button>
        </div>
      ...
      <div className="absolute top-28 left-4 z-[1000] flex flex-col gap-2">
        <div className="bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-2 flex flex-col gap-1">
          <button onClick={goToMyLocation} className="p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-emerald-400" title="موقعیت من">
            <Navigation className="h-4 w-4" />
          </button>
          <button onClick={() => { setMapCenter([32.5, 54.5]); setZoom(6); }} className="p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-emerald-400" title="نمای ایران">
            <Globe className="h-4 w-4" />
          </button>
        </div>

        <div className="bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-2 flex flex-col gap-1">
          <button onClick={() => startMeasure("distance")} className={`p-2 rounded-lg ${measureMode === "distance" ? "bg-emerald-600 text-white" : "text-slate-400 hover:bg-slate-800"}`} title="اندازه‌گیری مسافت">
            <Ruler className="h-4 w-4" />
          </button>
          <button onClick={() => startMeasure("area")} className={`p-2 rounded-lg ${measureMode === "area" ? "bg-emerald-600 text-white" : "text-slate-400 hover:bg-slate-800"}`} title="اندازه‌گیری مساحت">
            <Square className="h-4 w-4" />
          </button>
          <button onClick={() => startMeasure("radius")} className={`p-2 rounded-lg ${measureMode === "radius" ? "bg-emerald-600 text-white" : "text-slate-400 hover:bg-slate-800"}`} title="اندازه‌گیری شعاع">
            <Target className="h-4 w-4" />
          </button>
          {measureMode !== "none" && (
            <>
              <div className="border-t border-slate-700 my-1" />
              <button onClick={() => { if (measureResult) finishAndSaveMeasure(); }} className="p-2 rounded-lg text-emerald-400 hover:bg-slate-800" title="ذخیره">
        ...
        <div className="bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-2 flex flex-col gap-1">
          <button onClick={() => openSaveDialog()} className="p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-emerald-400" title="ثبت مکان">
            <Save className="h-4 w-4" />
          </button>
          <button onClick={() => setShowSavedPanel(!showSavedPanel)} className={`p-2 rounded-lg relative ${showSavedPanel ? "bg-emerald-600 text-white" : "text-slate-400 hover:bg-slate-800"}`} title={`مکان‌های من (${savedLocations.length})`}>
        ...
        <div className="bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-2 flex flex-col gap-1">
          <button onClick={() => setShowLayerPanel(!showLayerPanel)} className={`p-2 rounded-lg ${showLayerPanel ? "bg-emerald-600 text-white" : "text-slate-400 hover:bg-slate-800"}`} title="لایه‌ها">
            <Layers className="h-4 w-4" />
          </button>
          <button onClick={() => setShowHeatmap(!showHeatmap)} className={`p-2 rounded-lg ${showHeatmap ? "bg-emerald-600 text-white" : "text-slate-400 hover:bg-slate-800"}`} title="نقشه حرارتی">
            <Flame className="h-4 w-4" />
          </button>
          <button onClick={() => window.print()} className="p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-emerald-400" title="چاپ">
          ...
          <div className="relative group">
            ...
            <div className="absolute top-0 right-full mr-1 bg-slate-900 border border-slate-700 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-[1001] min-w-[180px]">
              ...
              <button onClick={exportHTMLReport} className="w-full px-3 py-2 text-right text-sm text-slate-300 hover:bg-slate-800 last:rounded-b-xl flex items-center gap-2">
      ...
      {showLayerPanel && (
        <div className="absolute top-28 right-4 z-[1000] bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-4 w-80 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            <p className="text-xs text-slate-400 mb-2 uppercase font-bold">لایه‌های هواشناسی</p>
            <div className="space-y-1">
              {Object.entries(WEATHER_LAYERS).map(([key, layer]) => (
                <button
                  key={key}
                  onClick={() => setWeatherLayers(prev => ({ ...prev, [key]: !prev[key] }))}
                  className={`w-full text-right px-3 py-2 rounded-lg text-sm ${
                    weatherLayers[key] ? "bg-slate-800 text-white" : "text-slate-400 hover:bg-slate-800"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        {key === "temperature" && <Thermometer className="h-3 w-3" style={{ color: layer.color }} />}
                        {key === "precipitation" && <CloudRain className="h-3 w-3" style={{ color: layer.color }} />}
                        {key === "wind" && <Wind className="h-3 w-3" style={{ color: layer.color }} />}
                        {key === "clouds" && <Cloud className="h-3 w-3" style={{ color: layer.color }} />}
                        {key === "humidity" && <Droplets className="h-3 w-3" style={{ color: layer.color }} />}
                        {key === "pressure" && <Gauge className="h-3 w-3" style={{ color: layer.color }} />}
                        <span>{layer.name}</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-1 mr-5">{layer.description}</p>
                    </div>
                    {weatherLayers[key] ? <Eye className="h-4 w-4 text-emerald-400" /> : <EyeOff className="h-4 w-4" />}
                  </div>
                </button>
              ))}
            </div>
            <p className="text-xs text-amber-400 mt-2">⚠️ نیاز به API Key از OpenWeatherMap</p>
          </div>
        </div>
      )}
      ...
      {showSavedPanel && (
        <div className="absolute top-28 right-4 z-[1000] bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-4 w-80 max-h-[80vh] overflow-y-auto">
          ...
          <button onClick={() => openSaveDialog()} className="w-full mb-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2">
                ...
                return (
                  <div key={loc.id} className="p-3 bg-slate-800/50 rounded-lg border border-slate-700">
                    <div className="flex items-start justify-between mb-1">
                      ...
                      <button onClick={() => handleDeleteLocation(loc.id)} className="text-slate-500 hover:text-red-400 p-1">
                        <Trash2 className="h-3 w-3" />
                      </button>
                    </div>
                    {loc.description && <p className="text-xs text-slate-300 mb-2">{loc.description}</p>}
                    <button onClick={() => flyToLocation(loc)} className="w-full py-1 bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-400 rounded text-xs font-bold">
                      نمایش روی نقشه
                    </button>
                  </div>
                );
        ...
        <div className="absolute inset-0 z-[2000] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-md w-full p-6">
                  ...
                  {Object.entries(LOCATION_CATEGORIES).map(([key, cat]) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-xs font-bold ${
                        saveForm.category === key ? "bg-emerald-600 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"
                      }`}
                    >
                      {cat.label}
                    </button>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={() => setShowSaveDialog(false)} className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-sm">
                انصراف
              </button>
              <button onClick={handleSaveLocation} className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold flex items-center justify-center gap-2">
...
# ========== Main ==========
def main():
    print("🌡️ افزودن نقشه‌های هواشناسی و حرارتی")
    print("=" * 70)
    ...
    update_production_gis()
    
    print("\n" + "=" * 70)
    print("✅ تغییرات اعمال شد!")
    print("\n🎯 تغییرات:")
    print("   1. ✅ کنترل زوم به سمت چپ (topleft)")
    print("   2. ✅ نقشه‌های هواشناسی:")
    print("      • 🌡️ دما (Temperature)")
    print("      • 🌧️ بارش (Precipitation)")
    print("      • 💨 باد (Wind)")
    print("      • ☁️ ابر (Clouds)")
    print("      • 💧 رطوبت (Humidity)")
    print("      • 📊 فشار (Pressure)")
    print("   3. ✅ نقشه حرارتی (Heatmap)")
    print("      • گرادیان رنگی علمی")
    print("      • آبی → سبز → زرد → نارنجی → قرمز")
    print("   4. ✅ آیکون‌های جدید:")
    print("      • Thermometer, Wind, CloudRain, Cloud, Droplets, Gauge")
    print("")
    print("📝 نکته:")
    print("   • لایه‌های هواشناسی نیاز به API Key از OpenWeatherMap دارند")
    print("   • برای دریافت کلید: https://openweathermap.org/api")
    print("   • در کد، به جای YOUR_API_KEY، کلید خود را قرار دهید")
    print("")
    print("🚀 گام بعدی:")
    print("   1. پاک‌سازی کش:")
    print("      cd apps\\web")
    ...
    print("   4. روی آیکون 🔥 (نقشه حرارتی) کلیک کنید")
    print("   5. در پنل لایه‌ها، لایه‌های هواشناسی را فعال کنید")
    print("=" * 70)
