...
"""
import os, sys, json, re, ast, hashlib
...
IGNORE_DIRS = {'.git', 'node_modules', '.next', '.venv', '__pycache__', 
               'tutorial_env', '.pytest_cache', '.mypy_cache', 'dist', 'build'}
...
class BaseAnalyzer:
    ...
    def add_finding(self, severity: str, category: str, message: str, path: str = "", suggestion: str = ""):
...
class StructureAnalyzer(BaseAnalyzer):
    ...
    def analyze(self):
        ...
        if file_count > 0:
            self.add_finding("success", "structure", 
                           f"پروژه شامل {file_count} فایل در {dir_count} پوشه است")
        if large_files:
            self.add_finding("warning", "size",
                           f"{len(large_files)} فایل بزرگ (>500KB) یافت شد")
        if deep_paths:
            self.add_finding("info", "structure",
...
class DependencyAnalyzer(BaseAnalyzer):
    ...
    def analyze(self):
        ...
        if backend_deps.get("packages"):
            self.add_finding("success", "backend",
                           f"{len(backend_deps['packages'])} پکیج پایتون نصب شده")
        if frontend_deps.get("packages"):
            self.add_finding("success", "frontend",
                           f"{len(frontend_deps['packages'])} پکیج Node.js نصب شده")
        ...
        for req_file in self.root.rglob("requirements*.txt"):
            ...
            try:
                ...
                result["requirements_files"].append({
                    "path": str(req_file.relative_to(self.root)),
                    "packages": packages
                })
                for pkg in packages:
                    if pkg:
...
class CodeQualityAnalyzer(BaseAnalyzer):
    ...
    def _analyze_python(self) -> dict:
        stats = {"files": 0, "total_lines": 0, "empty_lines": 0, 
                 "comment_lines": 0, "functions": 0, "classes": 0,
                 "complex_files": []}
        ...
        if stats["complex_files"]:
            self.add_finding("warning", "complexity",
    ...
    def analyze(self):
        ...
        for path in self.root.rglob('*'):
            ...
            if path.suffix.lower() in SENSITIVE_EXTENSIONS:
                ...
                self.add_finding("warning", "security",
            ...
            if path.suffix.lower() in {'.py', '.ts', '.tsx', '.js', '.json', '.env', '.yml', '.yaml'}:
                try:
                    content = path.read_text(encoding="utf-8", errors="ignore")
                    for pattern in SENSITIVE_PATTERNS:
                        # الگوهای مشکوک
                        if re.search(rf'{pattern}\s*[:=]\s*["\'][^"\']{{8,}}["\']', content, re.IGNORECASE):
                            exposed_secrets.append({
                                "file": rel_path,
                                "pattern": pattern
                            })
                            self.add_finding("critical", "security",
...
class BackendAnalyzer(BaseAnalyzer):
    ...
    def analyze(self):
        ...
        self.add_finding("success" if endpoints else "warning", "backend",
...
class FrontendAnalyzer(BaseAnalyzer):
    ...
    def analyze(self):
        ...
        self.add_finding("success" if pages else "warning", "frontend",
...
class ReportGenerator:
        ...
        for analyzer in self.analyzers:
            ...
            if analyzer.findings:
                ...
                for f in analyzer.findings[:15]:
                    icon = {"critical": "🔴", "warning": "🟡", "info": "🔵", "success": "🟢"}.get(f["severity"], "⚪")
    ...
    def generate_json_report(self) -> dict:
        return {
            ...
            "root": str(ROOT),
            "summary": {
                "critical": sum(1 for a in self.analyzers for f in a.findings if f["severity"] == "critical"),
                "warning": sum(1 for a in self.analyzers for f in a.findings if f["severity"] == "warning"),
                "success": sum(1 for a in self.analyzers for f in a.findings if f["severity"] == "success"),
            },
            "analyzers": {
                analyzer.name: {
