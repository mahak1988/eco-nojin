...
CROPS = {
    "گندم": {
        ...
        "soil_requirements": ["loam", "silt_loam"],
    },
    "جو": {
        "yield_base": 1.1,
        "price_per_ton": 9000,
        ...
        "soil_requirements": ["loam", "sandy_loam"],
    },
    "ذرت": {
        "yield_base": 8.5,
        "price_per_ton": 15000,
        ...
        "soil_requirements": ["loam", "clay_loam"],
    },
    "برنج": {
        "yield_base": 4.2,
        "price_per_ton": 45000,
        ...
        "soil_requirements": ["clay", "clay_loam"],
    },
    "پنبه": {
        "yield_base": 2.8,
        "price_per_ton": 85000,
        ...
        "climate_sensitivity": 0.35,
        "soil_requirements": ["loam", "sandy_loam"],
    },
    "زعفران": {
        "yield_base": 0.025,
        "price_per_ton": 80000000,
        "water_need_mm": 300,
        "cycle_days": 240,
        "planting_season": "autumn",
        "market_volatility": 0.35,
        "climate_sensitivity": 0.2,
        "soil_requirements": ["sandy_loam", "loam"],
    },
    "پسته": {
        "yield_base": 0.8,
        "price_per_ton": 450000,
        "water_need_mm": 600,
        "cycle_days": 365,
        "planting_season": "perennial",
...
class EconomicSimulator:
    ...
    def monte_carlo(area_ha: float, crop: str, inputs: Dict, iterations: int = 1000) -> Dict:
        ...
        for _ in range(iterations):
            ...
            price_var = crop_data["price_per_ton"] * (
                1 + random.gauss(0, crop_data["market_volatility"])
            )
            revenue = area_ha * yield_var * price_var

            # هزینه‌ها با نوسان جزئی
            water_cost = (
                inputs.get("water_cost_per_mm", 3000)
                * crop_data["water_need_mm"]
                * area_ha
                * random.uniform(0.9, 1.1)
            )
            labor_cost = (
                inputs.get("labor_cost_per_ha", 2000000) * area_ha * random.uniform(0.95, 1.05)
            )
            input_cost = (
                inputs.get("input_cost_per_ha", 800000) * area_ha * random.uniform(0.9, 1.1)
            )
            total_cost = water_cost + labor_cost + input_cost

            # ریسک اقلیمی
            climate_factor = 1 - inputs.get("climate_risk", 0.2) * crop_data[
                "climate_sensitivity"
    ...
    def sensitivity_analysis(base_params: Dict, param: str, range_pct: float = 20) -> List[Dict]:
        ...
        for pct in range(-int(range_pct), int(range_pct) + 1, 5):
            ...
            profit = EconomicSimulator.calculate_profit(
                modified.get("area_ha", 10), modified.get("crop", "گندم"), modified
            )
            results.append(
                {
    ...
    def optimize_crop_mix(regions: List[str], budget: float, risk_tolerance: float) -> Dict:
        ...
        for crop in CROPS:
            ...
            expected_yield = crop_data["yield_base"] * (
                1 - risk_tolerance * crop_data["climate_sensitivity"]
            )
            expected_price = crop_data["price_per_ton"] * (
                1 - risk_tolerance * crop_data["market_volatility"]
            )
            expected_profit = expected_yield * expected_price - (
                crop_data["water_need_mm"] * 3000 + 2800000
            )
            ...
            if expected_profit > best_solution["profit"]:
                best_solution = {
                    ...
                    "allocation": {
                        region: {
                            "crop": crop,
                            "area_ha": budget / len(regions) / crop_data["price_per_ton"],
                        }
                        for region in regions
                    },
                    "total_water_need": sum(CROPS[crop]["water_need_mm"] for _ in regions),
                    "risk_score": round(
                        risk_tolerance * crop_data["climate_sensitivity"]
                        + (1 - risk_tolerance) * crop_data["market_volatility"],
                        3,
                    ),
                }
        ...
        "regions": [
            {
                ...
                **data,
                "supported_crops": [
                    c
                    for c in CROPS
                    if data["climate"] in ["semi-arid", "arid"] or c in ["برنج", "مرکبات"]
                ],
            }
...
async def simulate_profit(data: SimulateProfitInput):
    result = EconomicSimulator.calculate_profit(
        ...
        data.crop,
        {
            "water_cost_per_mm": data.water_cost_per_mm,
            "labor_cost_per_ha": data.labor_cost_per_ha,
            "input_cost_per_ha": data.input_cost_per_ha,
...
async def simulate_montecarlo(
    data: SimulateProfitInput, iterations: int = Query(1000, ge=100, le=5000)
):
...
async def sensitivity_analysis(
    ...
    param: str = Query(
        ...,
        description="پارامتر: area_ha, water_cost_per_mm, labor_cost_per_ha, input_cost_per_ha, climate_risk",
    ),
...
async def optimize_crop_mix(
    ...
    risk_tolerance: float = Body(
        0.3, ge=0, le=1, description="تحمل ریسک: 0=محافظه‌کار، 1=ریسک‌پذیر"
    ),
):
...
async def analyze_stream(query: str = Body(...), region: str = Body("خراسان رضوی")):
    ...
    async def run_analysis():
        events = [
            ...
            {"event_type": "task_start", "task_id": "ndvi", "message": "🛰️ تحلیل NDVI شروع شد"},
            {
                ...
                "tool": "gee_ndvi",
                "message": "✅ NDVI محاسبه شد",
                "data": {"value": round(random.uniform(0.3, 0.7), 3)},
            },
            {
                "event_type": "tool_executed",
                "tool": "open_meteo",
                "message": "✅ داده‌های هواشناسی دریافت شد",
                "data": {"rainfall_30d": round(random.uniform(10, 80), 1)},
            },
            {
                "event_type": "task_complete",
                "task_id": "ndvi",
                "message": "✅ تحلیل پوشش گیاهی تکمیل شد",
            },
            {
                "event_type": "task_start",
                "task_id": "economic",
                "message": "💰 تحلیل اقتصادی شروع شد",
            },
            {
                "event_type": "tool_executed",
                "tool": "economic_simulator",
                "message": "✅ شبیه‌سازی اقتصادی انجام شد",
                "data": {"profit_estimate": round(random.uniform(5e6, 5e7), 0)},
            },
            {
                "event_type": "task_complete",
                "task_id": "economic",
                "message": "✅ تحلیل اقتصادی تکمیل شد",
            },
            {
                "event_type": "node_start",
                "node_name": "finalizer",
                "message": "📝 تولید گزارش نهایی...",
            },
            {
                "event_type": "final",
    ...
    return {
        ...
        "status": "started",
        "websocket_url": f"/ws/analyze/{session_id}",
        "region": region,
    }
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        await websocket.send_json(
            {"event_type": "connected", "session_id": session_id, "message": "✅ اتصال برقرار شد"}
        )
        while True:
            try:
...
async def validation_error(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(status_code=422, content={"error": "درخواست نامعتبر", "details": errors})
