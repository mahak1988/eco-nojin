...
class LocalVectorMemory:
    ...
    def _save_data(self):
        ...
        with open(self.storage_path, 'w', encoding='utf-8') as f:
            json.dump({'vectors': [v.tolist() for v in self.vectors],
                      'metadata': self.metadata}, f, ensure_ascii=False, indent=2)
    ...
    def search(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        for i, vector in enumerate(self.vectors):
            sim = np.dot(query_embedding, vector) / (np.linalg.norm(query_embedding) * np.linalg.norm(vector))
            similarities.append((sim, i))
        similarities.sort(reverse=True)
        return [{'text': self.metadata[idx]['text'], 'score': float(score),
