...
from api.database import FieldAnalysis, FieldObservation, MRVReport
...
class AnalysisRepository:
    ...
    async def get_ndvi_timeseries(self, region: str, days: int = 180) -> List[Dict]:
        ...
        query = (
            select(FieldAnalysis.created_at, FieldAnalysis.ndvi_avg, FieldAnalysis.ndvi_health)
        ...
        result = await self.session.execute(query)
        return [
            {"date": row[0].isoformat(), "ndvi": row[1], "health": row[2]} for row in result.all()
        ]
