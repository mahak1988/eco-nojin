...
def fix_polygon_rpc():
    ...
    for file_path in files_to_update:
        ...
        try:
            ...
            content = re.sub(
                r'https?://rpc-mainnet\.matic\.vigil\.sh',
                recommended_rpc,
                content
            )
    ...
    for pattern in patterns:
        for file_path in FRONTEND_DIR.glob(pattern):
            try:
                ...
                if 'useParams' in content and "'use client'" not in content and '"use client"' not in content:
                    issues_found.append((rel_path, "useParams needs 'use client' directive"))
                
                # 2. useState/useEffect بدون 'use client'
                if ('useState' in content or 'useEffect' in content) and \
                   "'use client'" not in content and '"use client"' not in content:
                    issues_found.append((rel_path, "useState/useEffect needs 'use client'"))
                
                # 3. استفاده از params بدون await (در Server Components)
                if 'params' in content and 'await params' not in content and \
                   "'use client'" not in content and '"use client"' not in content and \
                   'async function' in content:
                    # بررسی اینکه آیا params destructured شده بدون await
                    if re.search(r'const\s+\{[^}]+\}\s*=\s*params(?!\s*;)', content):
                        issues_found.append((rel_path, "params should be awaited in Server Component"))
                ...
                if use_client_idx is not None and react_import_idx is not None:
                    if react_import_idx < use_client_idx:
                        issues_found.append((rel_path, "'use client' must be before 'import React'"))
