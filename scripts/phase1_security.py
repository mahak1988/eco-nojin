...
class EnvVal(BaseChecker):
    ...
    def run(self) -> Result:
        ...
        if not ep.exists():
            findings.append(
                Finding(
                    ...
                    None,
                    None,
                    "Create .env based on template and fill real values",
                )
            )
        ...
        try:
                ...
                if k not in ev:
                    findings.append(
                        Finding(
                            "MEDIUM",
                            "EnvMissingKey",
                            f"Required key {k} ({d}) not defined",
                            ".env",
                            None,
                            f"Add {k}=real_value to .env",
                        )
                    )
            pc = sum(
                1 for k, v in ev.items() if any(x in k.upper() for x in self.SENS) and self._ph(v)
            )
            for k, v in ev.items():
                if any(x in k.upper() for x in self.SENS) and self._ph(v):
                    findings.append(
                        Finding(
                            "INFO",
                            "EnvPlaceholder",
                            f"Sensitive key {k} has placeholder value",
                            ".env",
                            None,
                            "Replace with real value before production",
                        )
                    )
            score = 1.0
            ...
            return Result(
                "EnvVal",
                "PASS" if score >= 0.8 else "WARN" if score >= 0.5 else "FAIL",
                score,
                findings,
                {"exists": True, "total": len(ev), "placeholder": pc},
            )
        except Exception as e:
            return Result(
                "EnvVal",
                "FAIL",
                0.0,
                [Finding("CRITICAL", "EnvError", f"Error reading .env: {e}")],
...
class SBOM(BaseChecker):
    def run(self) -> Result:
                ...
                if fp.name == "requirements.txt":
                    ...
                    if sec:
                        deps = [
                            {"name": m, "version": "unknown"}
                            for m in re.findall(
                                r"^\s*([a-zA-Z0-9_-]+)\s*[=<>]", sec.group(1), re.MULTILINE
                            )
                        ]
                break
            except Exception:
        ...
        if not deps:
            findings.append(
                Finding(
                    "MEDIUM",
                    "SBOM",
                    "No valid dependency file found",
                    ".",
                    "",
        ...
        return Result(
            "SBOM",
            "PASS" if score >= 0.7 else "WARN" if score >= 0.4 else "FAIL",
            score,
            findings,
            {"total": len(deps), "at": datetime.now(timezone.utc).isoformat()},
        )
...
class SecretScan(BaseChecker):
    ...
    def run(self) -> Result:
        ...
        for fp in self.root.rglob("*"):
            try:
                if (
                    not fp.is_file()
                    or self._ign(fp)
                    or fp.suffix.lower() not in exts
                    or fp.stat().st_size > 500 * 1024
                ):
                    skipped += 1
                    continue
                try:
                        ...
                        if m:
                            ...
                            findings.append(
                                Finding(
                                    "CRITICAL",
                                    f"Secret/{cat}",
                                    f"Secret pattern detected ({cat})",
                                    rp,
                                    ln_n,
                                    "Remove secret and use os.getenv() + Vault",
                                )
                            )
                            break
            except Exception:
        ...
        return Result(
            "SecretScan",
            "FAIL" if score == 0.0 else "WARN" if score < 1.0 else "PASS",
            score,
            findings,
            {"scanned": scanned, "skipped": skipped, "time": round(time.time() % 3600, 2)},
        )
...
class mTLS(BaseChecker):
    def run(self) -> Result:
        ...
        if not certs:
            findings.append(
                Finding(
                    "INFO",
                    "mTLS",
                    "No TLS certificate found (may use SPIFFE/SPIRE)",
                    None,
                    None,
                    "Ensure mTLS is implemented for Zero Trust",
                )
            )
            score = 0.9
        else:
            for cp in certs:
                try:
                    ...
                    if openssl_ok:
                        rc, out, _ = self._cmd(
                            ["openssl", "x509", "-enddate", "-noout", "-in", str(cp)], timeout=10
                        )
                                    ...
                                    if dl < 0:
                                        ...
                                        findings.append(
                                            Finding(
                                                "HIGH",
                                                "mTLS",
                                                f"Certificate expiring soon ({dl} days): {cp.name}",
                                                rp,
                                                None,
        ...
        return Result(
            "mTLS",
            "FAIL" if score < 0.6 else "WARN" if score < 1.0 else "PASS",
            score,
            findings,
            {"certs": len(certs), "openssl": openssl_ok, "platform": sys.platform},
        )
...
class Pipeline:
    def __init__(self, root: Path, fail_on_critical: bool = True):
        ...
        self.checks = [
            EnvVal(self.root, {}),
            SBOM(self.root, {}),
            SecretScan(self.root, {}),
            mTLS(self.root, {}),
        ]

    def run(self) -> Dict:
        logger.info(
            f"Starting Phase 1 Security Pipeline | Root: {self.root} | Platform: {sys.platform}"
        )
        results = []
        has_crit = False
        t0 = time.time()
        ...
        report = {
            ...
            "next": [
                f"Fix: {r['check_name']} - {r['findings'][0]['message']}"
                for r in results
                if r["status"] == "FAIL"
            ]
            or [
                f"Improve: {r['check_name']} - {r['findings'][0]['recommendation']}"
                for r in results
                if r["status"] == "WARN"
            ]
        ...
        if self.fail_on_critical and has_crit:
            ...
            sys.exit(1)
        logger.info(
            f"Pipeline complete | Score: {overall:.2f} | Status: {report['status']} | Time: {elapsed:.1f}s"
        )
        return report
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Phase 1 Security Pipeline - Single file, Windows-compatible"
    )
