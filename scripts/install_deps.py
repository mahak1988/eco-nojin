...
class DependencyInstaller:
    ...
    CATEGORIES = {
        ...
        "database": ["SQLAlchemy", "psycopg2-binary", "GeoAlchemy2", "alembic"],
        "scientific": [
            "numpy",
            "pandas",
            "scipy",
        ...
        "development": ["black", "isort", "flake8", "mypy", "ruff", "bandit", "safety"],
        "testing": [
            "pytest",
            "pytest-asyncio",
            "pytest-cov",
        ...
        "utilities": ["requests", "tqdm", "rich", "click", "typer", "tenacity"],
    }
    ...
    def create_virtualenv(self) -> bool:
        ...
        try:
            SafeSubprocess.run(["python", "-m", "venv", "--clear", str(self.venv_path)])
    ...
    def install_all(self) -> Dict[str, Tuple[int, int]]:
        ...
        for category in install_order:
            if category in self.CATEGORIES:
                results[category] = self.install_category(category, self.CATEGORIES[category])
