...
class EconojinFixer:
    def __init__(self):
        ...
        self.report_file = (
            self.project_root / f"fixer_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
    ...
    def step4_fix_temp_files(self):
        ...
        # ایجاد پوشه backup
        backup_dir = (
            self.project_root / f"_syntax_error_backup_{datetime.now().strftime('%Y%m%d_%H%M')}"
        )
    ...
    def step6_check_missing_packages(self):
        ...
        if missing:
            ...
            print(
                f"   {self.python_exe} -m pip install -r {req_file} --trusted-host pypi.org --trusted-host files.pythonhosted.org"
            )
            print(f"\n💡 یا با mirror ایرانی:")
            print(
                f"   {self.python_exe} -m pip install -r {req_file} -i https://mirror-pypi.runflare.com/simple/ --trusted-host mirror-pypi.runflare.com"
            )
        else:
