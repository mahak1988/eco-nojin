    ...
    def convert_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                match = re.match(r"^(\s*)print\s*\(\s*(.+?)\s*\)(\s*)$", line)
                ...
                if match:
                    ...
                    if not added_import:
                        ...
                        new_lines.insert(
                            import_idx, "from scripts.core.logger import UnifiedLogger"
                        )
                        new_lines.insert(
                            import_idx + 1, "logger = UnifiedLogger.get_logger(__name__)"
                        )
            ...
            if content != original:
                ...
                logger.info(
                    f"  ✅ Converted {file_path.name}: {sum(changes.values())} print → logging"
                )
                return {"file": str(file_path), "changes": changes, "success": True}
