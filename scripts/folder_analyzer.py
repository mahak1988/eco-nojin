...
class FolderStructureAnalyzer:
        ...
        if files:
            ...
            if len(files) > 20:
                self.stats["large_folders"].append(
                    {"path": str(current_path.relative_to(self.project_path)), "files": len(files)}
                )
    ...
    def _generate_report(self):
        ...
        self._log(f"|---|---|")
        self._log(
            f"| 📁 تعداد کل پوشه‌ها (بدون node_modules و...) | **{self.stats['total_folders']}** |"
        )
        ...
        sorted_categories = sorted(
            self.stats["folder_categories"].items(), key=lambda x: x[1]["count"], reverse=True
        )
        ...
        if self.stats["large_folders"]:
            ...
            self._log("=" * 70)
            sorted_large = sorted(
                self.stats["large_folders"], key=lambda x: x["files"], reverse=True
            )
        ...
        for ext, count in sorted_extensions:
            percentage = (
                (count / self.stats["total_files"] * 100) if self.stats["total_files"] > 0 else 0
            )
        ...
        if len(self.stats["empty_folders"]) > 5:
            recommendations.append(
                f"- 🧹 **پاکسازی:** {len(self.stats['empty_folders'])} پوشه خالی وجود دارد که می‌توان حذف کرد."
            )
        if self.stats["max_depth"] > 7:
            recommendations.append(
                f"- 📐 **بازسازی:** عمق ساختار ({self.stats['max_depth']} لایه) زیاد است. پیشنهاد می‌شود ساختار را مسطح‌تر کنید."
            )
        if len(self.stats["uncategorized_folders"]) > 10:
            recommendations.append(
                f"- 🏷️ **سازماندهی:** {len(self.stats['uncategorized_folders'])} پوشه بدون دسته‌بندی مشخص وجود دارد. بررسی ساختار نام‌گذاری توصیه می‌شود."
            )

        large_folder_count = sum(1 for x in self.stats["large_folders"] if x["files"] > 50)
        if large_folder_count > 0:
            recommendations.append(
                f"- 📦 **تفکیک:** {large_folder_count} پوشه با بیش از ۵۰ فایل وجود دارد. پیشنهاد می‌شود آن‌ها را به زیرپوشه‌های کوچک‌تر تقسیم کنید."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "project_path", nargs="?", default=".", help="مسیر ریشه پروژه (پیش‌فرض: مسیر فعلی)"
    )
    parser.add_argument("-o", "--output", help="مسیر فایل خروجی گزارش")
