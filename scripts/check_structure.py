...
class StructureChecker:
    ...
    def print_report(self, report: CheckReport):
        ...
        if report.optional_missing:
            print(f"{Colors.YELLOW}{Colors.BOLD}ℹ️ موارد اختیاری مفقوده (تکمیل پیشنهادی):{Colors.END}")
        ...
        if report.missing == 0:
            ...
            print(f"{Colors.YELLOW}{Colors.BOLD}✅ ساختار اصلی کامل است. موارد اختیاری را می‌توانید بعداً اضافه کنید.{Colors.END}")
        else:
            print(f"{Colors.RED}{Colors.BOLD}❌ ساختار ناقص است. لطفاً موارد حیاتی را ایجاد کنید.{Colors.END}")
