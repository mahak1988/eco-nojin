...
class ImpactScorer:
    ...
    def calculate(
        ...
        biodiversity_score = self._score_biodiversity(activity_type, species, biodiversity_index)
        ...
        logger.info(f"Impact score: {total:.1f}/100 ({grade}), " f"bonus: {bonus:.2f}x")

        return score

    def _score_carbon(
        self, carbon_kg: float, area_hectares: float, species: Optional[str]
    ) -> float:
        """امتیاز کربن (0-100)"""
        if area_hectares <= 0:
            return 0.0
    ...
    def _score_biodiversity(
        self, activity_type: str, species: Optional[str], biodiversity_index: float
    ) -> float:
        """امتیاز تنوع زیستی"""
    ...
    def compare_activities(self, activities: List[Dict]) -> List[Dict]:
        """مقایسه چند فعالیت"""
