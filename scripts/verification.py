...
class ScientificVerifier:
    ...
    async def verify_activity(
        ...
        sci_result = await self._verify_scientific(activity_type, location, claimed_carbon_kg)
        ...
        logger.info(f"Verification complete: {verified} (confidence: {confidence:.2f})")
    ...
    async def _verify_scientific(
        ...
        try:
            from core.gaia.calculator import ActivityType, CarbonCalculator, ClimateData, Location
