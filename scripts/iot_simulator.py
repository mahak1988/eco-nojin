...
class IoTSimulator:
    ...
    def generate_reading(self, sensor_id: str) -> Optional[SensorReading]:
        ...
        if sensor_type == "air_temperature":
            ...
            seasonal = (
                15
                + 15
                * (1 + __import__("math").sin(2 * __import__("math").pi * (day_of_year - 80) / 365))
                / 2
            )
            # Daily variation
            ...
            base_value = 50 - 10 * __import__("math").sin(
    ...
    def get_statistics(self) -> Dict:
        ...
        avg_battery = (
            sum(s["battery"] for s in self.sensors.values()) / len(self.sensors)
            if self.sensors
            else 0
        )
            ...
            growth_data.append(
                {
                    ...
                    "timestamp": (
                        datetime.now(timezone.utc) - timedelta(days=days - day)
                    ).isoformat(),
                }
            )
