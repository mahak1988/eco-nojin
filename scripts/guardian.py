...
IGNORE_DIRS = {
    '.venv', 'venv', 'env', 'tutorial_env', 'virtualenv',
    'node_modules', '.pnpm-store', 'pnpm-global',
    '__pycache__', '.pytest_cache', '.mypy_cache', '.ruff_cache',
    'build', 'dist', '.next', '.nuxt', 'artifacts', 'cache',
    'typechain-types', '.cache', '.parcel-cache', 'target',
    '.git', '.svn', '.idea', '.vscode', '.vs',
    'reports', 'offline_packages', 'data', 'datasets', 'uploads',
}

IGNORE_PATH_PARTS = ['site-packages', 'guardian.py', 'fix_security_findings.py', 'Lib/site-packages', '_backup_', '_cleanup_']
...
FALSE_POSITIVES = ['example', 'placeholder', 'your_', 'xxx', 'dummy', 
                   'test', 'mock', 'sk-test', 'REPLACE_ME', 'CHANGE_ME']
...
def scan_file(filepath: Path, findings: list):
    ...
    # Dangerous functions
    dangerous = {'eval': 'CRITICAL', 'exec': 'CRITICAL', 
                 'os.system': 'HIGH', 'pickle.loads': 'HIGH'}
...
def main():
    ...
    log("✓", "یافت شد: " + str(len(py_files)) + " Python + " + 
        str(len(sol_files)) + " Solidity (skipped: " + str(skipped) + ")", C.GREEN)
