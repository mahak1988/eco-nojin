...
def run_backend():
    venv_pip = BACKEND / ".venv" / "Scripts" / "python.exe" if os.name == "nt" else BACKEND / ".venv" / "bin" / "python"
    if not venv_pip.exists():
        print("❌ محیط مجازی بک‌اند یافت نشد. ابتدا 01_setup_econojin.py را اجرا کنید.")
        return
    cmd = [str(venv_pip), "-m", "uvicorn", "api.main:app", "--reload", "--host", "127.0.0.1", "--port", "8000"]
