        ...
        for fp in self.root.rglob("*"):
            ...
            try:
                if (
                    not fp.is_file()
                    or self._ign(fp)
                    or fp.suffix.lower() not in self.target_ext
                    or fp.stat().st_size > self.max_file_size
                ):
                    continue
...
class L2ScalabilityChecker(BaseChecker):
    def run(self) -> Result:
        ...
        counts, scanned = self._scan_patterns(patterns, 50)
        total = len(patterns)
        ...
        if counts["RollupArchitecture"] == 0:
            findings.append(
                Finding(
                    "MEDIUM",
                    "Missing",
                    "No L2/Rollup architecture detected",
                    None,
                    None,
                    "Define ZK or Optimistic Rollup strategy",
                )
            )
        if counts["ConsensusOptimization"] == 0:
            findings.append(
                Finding(
                    "LOW",
                    "Missing",
                    "No consensus/TPS targets found",
                    None,
                    None,
                    "Set finality <2s and TPS >10k targets",
                )
            )
        return Result(
            "L2Scalability",
        ...
        if counts["AgentPayments"] == 0:
            findings.append(
                Finding(
                    "MEDIUM",
                    "Missing",
                    "No agent payment pattern detected",
                    None,
                    None,
                    "Implement ERC-4337 for agent payments",
                )
            )
        if counts["TokenomicsDesign"] == 0:
            findings.append(
                Finding(
                    "LOW",
                    "Missing",
                    "No tokenomics model found",
                    None,
                    None,
                    "Define minting/burning/vesting schedules",
                )
            )
        return Result(
            "TokenEconomy",
        ...
        if counts["VulnerabilityProtection"] == 0:
            findings.append(
                Finding(
                    "HIGH",
                    "Missing",
                    "No reentrancy/overflow protection",
                    None,
                    None,
                    "Implement ReentrancyGuard and overflow checks",
                )
            )
        if counts["AuditAutomation"] == 0:
            findings.append(
                Finding(
                    "MEDIUM",
                    "Missing",
                    "No automated audit pattern found",
                    None,
                    None,
                    "Integrate Slither/Foundry fuzzing in CI/CD",
                )
            )
        return Result(
        ...
        if counts["KyberKEM"] == 0 and counts["DilithiumSig"] == 0:
            findings.append(
                Finding(
                    "HIGH",
                    "Missing",
                    "No PQC patterns detected",
                    None,
                    None,
                    "Plan migration to Kyber/Dilithium per NIST 2026",
                )
            )
        if counts["CryptoAgility"] == 0:
            findings.append(
                Finding(
                    "MEDIUM",
                    "Missing",
                    "No crypto-agility pattern found",
                    None,
                    None,
...
class Pipeline:
    ...
    def run(self) -> Dict:
        logger.info(f"Starting Phase 3 | Root: {self.root}")
        results = []
        has_crit = False
        t0 = time.time()
        ...
        report = {
            ...
            "next": (
                ...
                or ["System ready for deployment"]
            ),
        }
        ...
        if self.fail_on_critical and has_crit:
            logger.error("Critical finding - stopped")
            sys.exit(1)
        logger.info(f"Complete | Score: {overall:.2f} | Status: {report['status']}")
        return report
...
def main():
    ...
    if not args.root.exists():
        logger.error(f"Path not found: {args.root}")
        sys.exit(2)
