...
class EcoNojinAuditor:
    def __init__(self, local_dir, live_url=None):
        ...
        # معماری مورد انتظار بر اساس کتابچه فنی اکو نوژین
        self.expected_stack = {
    ...
    def analyze_local_architecture(self):
        ...
        # فرمت‌های کد مرتبط با پلتفرم
        code_extensions = {'.py', '.js', '.jsx', '.ts', '.tsx', '.vue', '.html', '.css', '.sql', '.sh'}
        
        for root, dirs, files in os.walk(self.local_dir):
            # حذف پوشه‌های غیرضروری مثل node_modules و .git برای سرعت بیشتر
            ...
            for file in files:
                ...
                # 2. تحلیل فایل‌های پیکربندی (Dependencies)
    ...
    def security_scan(self):
        ...
        # بررسی هاردکد شدن کلیدها و پسوردها در کدهای پایتون و JS
        sensitive_patterns = [
    ...
    def live_site_audit(self):
        ...
        try:
            ...
            headers = {
                'User-Agent': 'EcoNojinAuditor/1.0 (GIS & Carbon Platform Audit)'
            }
            ...
            for header, desc in security_headers.items():
                if header in response.headers:
                    ...
                    self.log(f"  {desc}: یافت نشد! (نیاز به تنظیم در Nginx/FastAPI) ⚠️", "WARNING")

            # بررسی سئو و متا تگ‌ها
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            for line in self.report_lines:
                # تبدیل ایموجی‌ها به فرمت مناسب مارک‌داون
                f.write(line.replace("✅", "- ✅").replace("⚠️", "- ⚠️").replace("❌", "- ❌") + "\n")
        
        self.log(f"✅ گزارش با موفقیت در مسیر زیر ذخیره شد:\n{report_path}", "SUCCESS")


if __name__ == "__main__":
    # تنظیمات مسیر و دامنه
    LOCAL_PATH = r"D:\econojin.com"  # مسیر محلی پروژه شما
    LIVE_URL = "https://econojin.com"  # در صورت آنلاین بودن سایت، این خط را فعال کنید
    
    print("🚀 شروع ممیزی جامع پلتفرم اکو نوژین (Eco Nojin)...\n")
