...
class PlantingPlan(Base):
    ...
    crop_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    ...
    expected_yield_t_ha: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class FarmTask(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    farm_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    planting_plan_id: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(255), nullable=False)
