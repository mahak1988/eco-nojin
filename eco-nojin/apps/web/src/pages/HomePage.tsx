...
const MODULE_LINKS = ["/satellite", "/simulators", "/simulators", "/simulators", "/mrv", "/simulators"];
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-[1.05fr_.95fr] gap-12 lg:gap-14 items-center relative z-10">
          ...
          <SectionReveal delay={300}>
            <div className="grid sm:grid-cols-2 gap-4" style={{ transform: "translateY(calc(var(--sy,0px) * -0.05))" }}>
              <Link to="/satellite" className="block h-full"><SatellitePanel /></Link>
              <Link to="/satellite" className="block h-full"><WeatherPanel /></Link>
            </div>
          </SectionReveal>
        </div>
      </section>
            ...
            {t.modules.map((m, i) => {
              ...
              return (
                <SectionReveal key={m.n} delay={i * 70} className={i === 0 ? "sm:col-span-2 sm:row-span-2" : i === 5 ? "sm:col-span-2" : ""}>
                  <Link to={MODULE_LINKS[i] ?? "/"} className={`group h-full min-h-[160px] rounded-[var(--r-lg)] border border-[var(--border-subtle)] bg-gradient-to-br ${g} p-6 flex flex-col justify-between card-hover cursor-pointer relative overflow-hidden`}>
