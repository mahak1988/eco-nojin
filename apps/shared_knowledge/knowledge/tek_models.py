...
class HistoricalPattern(Base):
    ...
    recommendation_template_fa: Mapped[str] = mapped_column(
    ...
    match_results: Mapped[list["TEKMatchResult"]] = relationship(
        "TEKMatchResult", back_populates="pattern", cascade="all, delete-orphan"
    )
    __table_args__ = (
        Index("ix_patterns_climate_category", "problem_category", "is_active"),
        Index("ix_patterns_age_score", "age_years", "success_score"),
    )

    def __repr__(self):
        return f"<HistoricalPattern(id={self.pattern_id}, civ='{self.civilization}', age={self.age_years}y)>"
...
class TEKMatchResult(Base):
    ...
    pattern_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey("historical_patterns.id", ondelete="CASCADE"),
        index=True,
        nullable=False,
    )
    farm_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("farms.id", ondelete="SET NULL"), index=True, nullable=True
    )
    latitude: Mapped[float] = mapped_column(Float, nullable=False)
    longitude: Mapped[float] = mapped_column(Float, nullable=False)
    climate_zone: Mapped[str | None] = mapped_column(String(10), nullable=True)
    annual_rainfall_mm: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    frost_risk: Mapped[bool | None] = mapped_column(Boolean, nullable=True)
    ...
    pattern: Mapped["HistoricalPattern"] = relationship(
        "HistoricalPattern", back_populates="match_results"
    )
    __table_args__ = (Index("ix_tek_match_farm", "farm_id", "pattern_id"),)

    def __repr__(self):
        return f"<TEKMatchResult(pattern={self.pattern_id}, score={self.match_score:.2f})>"
