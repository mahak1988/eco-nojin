...
class KnowledgeRepository:
    ...
    async def search_articles(
        ...
        stmt = select(KnowledgeArticle).where(
            and_(KnowledgeArticle.agent_type == agent_type, KnowledgeArticle.is_active == True)
        )
        ...
        stmt = stmt.order_by(
            KnowledgeArticle.priority.desc(), KnowledgeArticle.updated_at.desc()
        ).limit(limit)
    ...
    async def get_articles_by_category(
        self, agent_type: str, category: str
    ) -> list[KnowledgeArticle]:
    ...
    async def get_active_rules(self, agent_type: str) -> list[BusinessRule]:
        ...
        stmt = (
            select(BusinessRule)
            .where(and_(BusinessRule.agent_type == agent_type, BusinessRule.is_active == True))
    ...
    # Response Templates
    # ==========================================
    async def get_template(self, agent_type: str, intent: str) -> ResponseTemplate | None:
    ...
    async def save_memory(
        self, agent_type: str, user_id: int, memory_type: str, content: str, importance: float = 0.5
    ) -> AgentMemory:
        """ذخیره حافظه ایجنت."""
        memory = AgentMemory(
    ...
    async def get_user_memories(
        self, agent_type: str, user_id: int, limit: int = 10
    ) -> list[AgentMemory]:
        """دریافت حافظه‌های کاربر برای یک ایجنت."""
        stmt = (
            select(AgentMemory)
            .where(and_(AgentMemory.agent_type == agent_type, AgentMemory.user_id == user_id))
