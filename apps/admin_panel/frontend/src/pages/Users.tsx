...
import { Search, Shield, ShieldOff, CheckCircle2, XCircle, Trash2, Loader2, AlertCircle, RefreshCw } from 'lucide-react'
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage system users &bull; {users.length} total</p>
        </div>
        <button onClick={loadUsers} className="flex items-center gap-2 px-3 py-2 border rounded-lg hover:bg-accent text-sm">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)} className="px-4 py-2 border rounded-lg text-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-right p-4">Superuser</th>
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((user) => (
                    <tr key={user.id} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="p-4">
                        <span className="px-2 py-1 rounded-full bg-eco-100 text-eco-800 text-xs capitalize">
                          {user.role}
                        </span>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-1">
                          ...
                          >
                            {user.is_active ? <XCircle className="w-4 h-4 text-amber-600" /> : <CheckCircle2 className="w-4 h-4 text-green-600" />}
                          </button>
                          <button
                          ...
                          >
                            {user.is_superuser ? <ShieldOff className="w-4 h-4 text-purple-600" /> : <Shield className="w-4 h-4 text-purple-600" />}
                          </button>
                          {confirmDelete === user.id ? (
                            <div className="flex items-center gap-1">
                              <button onClick={() => handleDelete(user.id)} className="p-1.5 bg-red-100 text-red-700 rounded-lg text-xs font-medium">Confirm</button>
                              <button onClick={() => setConfirmDelete(null)} className="p-1.5 hover:bg-accent rounded-lg text-xs">Cancel</button>
                            </div>
                          ) : (
                            <button
