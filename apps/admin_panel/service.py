...
from apps.admin_panel.integrations.notifications import get_notification_service, init_notification_service, NotificationType, NotificationPriority
...
class AdminService:
    def __init__(self, session: AsyncSession) -> None:
        ...
        if get_cms_service() is None:
            init_cms_service(os.getenv("CMS_BASE_URL", "http://localhost:1337"), os.getenv("CMS_API_KEY"))
        if get_auth_service() is None:
            init_auth_service(os.getenv("AUTH_BASE_URL", "http://localhost:8000"), os.getenv("AUTH_API_KEY"))
        if get_ecommerce_service() is None:
            init_ecommerce_service(os.getenv("ECOMMERCE_BASE_URL", "http://localhost:8080"), os.getenv("ECOMMERCE_API_KEY"))
        if get_notification_service() is None:
            init_notification_service(os.getenv("NOTIFICATION_BASE_URL"), os.getenv("NOTIFICATION_API_KEY"))
    ...
    async def get_content_items_by_type(
        ...
        if search:
            content_items = [item for item in content_items if search.lower() in item.get("title", "").lower()]
    ...
    async def create_content_item(
        ...
        if cms_service:
            try:
                ...
                logger.warning(f"CMS service unavailable for creating content item, using mock: {e}")
    ...
    async def update_content_item(
        ...
        if cms_service:
            try:
                ...
                logger.warning(f"CMS service unavailable for updating content item, using mock: {e}")
                # Update in mock data
                content_items = await self.get_content_items_by_type(content_type)
    ...
    async def sync_content_to_modules(
        self,
        content_item: Optional[Dict[str, Any]],
        content_type: str,
        action: str,
        ...
        try:
                ...
                if ecommerce_service and content_item:
                    try:
                        if action == "create":
                            ...
                            await ecommerce_service.update_product(content_item.get("id"), content_item)
            ...
            await self.record_audit_event(
                event_type=f"content_sync_failed",
    ...
    async def analyze_user_behavior(self) -> Dict[str, Any]:
        ...
        most_active_users = sorted(user_activities.items(), key=lambda x: x[1]["count"], reverse=True)[:5]
        ...
        analysis = {
            "most_active_users": [{"email": user, "activity_count": data["count"]} for user, data in most_active_users],
        ...
        if most_common_events and most_common_events[0][1] > len(recent_logs) * 0.3:
            analysis["insights"].append(f"بیش از 30% فعالیت‌ها شامل {most_common_events[0][0]} هستند")

        if peak_activity_days and peak_activity_days[0][1] > sum(daily_patterns.values()) * 0.2:
            analysis["insights"].append(f"بیش از 20% فعالیت‌ها در روز {peak_activity_days[0][0]} اتفاق افتاده است")
    ...
    async def _calculate_active_users_trend(self) -> Dict[str, Any]:
        ...
        ).group_by(func.date_trunc('day', User.created_at)).order_by('date DESC').limit(30)
        ...
        if len(daily_counts) >= 2:
            ...
            trend_percentage = ((latest_count - previous_count) / previous_count * 100) if previous_count > 0 else 0
        else:
            trend_percentage = 0
    ...
    async def _analyze_content_growth(self) -> Dict[str, Any]:
        ...
        for content_type in content_types:
            ...
            growth_data[content_type["name"]] = {
                ...
                "recent_additions": len([item for item in content_items if self._is_recent(item.get("created_at"))])
            }
    ...
    async def _generate_predictions(self, dashboard_data: Dict, audit_logs: List[AuditLog]) -> Dict[str, Any]:
    ...
    async def _assess_system_risks(self, audit_logs: List[AuditLog]) -> Dict[str, Any]:
        ...
        failed_ops = [log for log in audit_logs if "failed" in log.event_type.lower() or "error" in log.event_type.lower()]
        ...
        admin_actions = [log for log in audit_logs if log.event_type in ["user.delete", "user.update", "setting.update"]]
    ...
    async def clear_cache(self, cache_type: Optional[str] = None) -> int:
        ...
        if cache_type is None:
            ...
            cleared_count = len(self._settings_cache) + len(self._content_types_cache) + len(self._content_items_cache)
        ...
        return [
            ...
            },
            {
                ...
                "fields": [
                    ...
                    {"name": "status", "type": "enumeration", "options": ["draft", "published", "out_of_stock"]}
                ],
                "created_at": datetime.utcnow().isoformat(),
                "updated_at": datetime.utcnow().isoformat()
            }
            ...
            return [
                {
                    ...
                    "title": "صفحه اصلی",
                    "slug": "home",
                    "content": {"blocks": [{"type": "paragraph", "data": {"content": "محتوای صفحه اصلی"}}]},
                    "status": "published",
                    "author_id": 1,
                ...
                },
                {
                    ...
                    "title": "درباره ما",
                    "slug": "about",
                    "content": {"blocks": [{"type": "paragraph", "data": {"content": "محتوای صفحه درباره ما"}}]},
                    "status": "published",
                    "author_id": 1,
                    "published_at": datetime.utcnow().isoformat(),
                    "created_at": datetime.utcnow().isoformat(),
            ...
            return [
                {
                    ...
                    "content": {"blocks": [{"type": "paragraph", "data": {"content": "محتوای مقاله"}}]},
                    "status": "published",
                    "author_id": 1,
                    "published_at": datetime.utcnow().isoformat(),
                    "created_at": datetime.utcnow().isoformat(),
    ...
    def _create_mock_content_item(self, content_type: str, data: Dict[str, Any], author_id: int) -> Dict[str, Any]:
