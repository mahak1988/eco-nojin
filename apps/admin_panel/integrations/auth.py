...
class AuthIntegrationService:
    ...
    def __init__(self, auth_base_url: str, api_key: str | None = None):
    ...
    async def get_user_profile(self, user_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            response = await self.client.get(
                f"{self.auth_base_url}/api/users/{user_id}", headers=headers
            )
    ...
    async def update_user_profile(
        ...
        try:
            ...
            response = await self.client.put(
                f"{self.auth_base_url}/api/users/{user_id}", headers=headers, json=profile_data
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_user_permissions(self, user_id: int) -> list[str]:
        ...
        try:
            headers = self._get_headers()
            response = await self.client.get(
                f"{self.auth_base_url}/api/users/{user_id}/permissions", headers=headers
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def revoke_user_role(self, user_id: int, role: str) -> bool:
        ...
        try:
            ...
            response = await self.client.delete(
                f"{self.auth_base_url}/api/users/{user_id}/roles/{role}", headers=headers
            )
            response.raise_for_status()
            return True
        except Exception as e:
...
auth_service: AuthIntegrationService | None = None


def get_auth_service() -> AuthIntegrationService | None:
    """Get the Auth integration service instance."""
    global auth_service
    return auth_service


def init_auth_service(base_url: str, api_key: str | None = None):
