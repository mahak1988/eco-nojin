...
class EcommerceIntegrationService:
    ...
    def __init__(self, ecommerce_base_url: str, api_key: str | None = None):
        self.ecommerce_base_url = ecommerce_base_url
        self.api_key = api_key
        self.client = httpx.AsyncClient(timeout=30.0)

    async def get_products(self, params: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        """Get products from e-commerce service."""
        try:
            headers = self._get_headers()
            response = await self.client.get(
                f"{self.ecommerce_base_url}/api/products", headers=headers, params=params
            )
    ...
    async def get_product(self, product_id: int) -> dict[str, Any] | None:
        ...
        try:
            headers = self._get_headers()
            response = await self.client.get(
                f"{self.ecommerce_base_url}/api/products/{product_id}", headers=headers
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def create_product(self, product_data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            response = await self.client.post(
                f"{self.ecommerce_base_url}/api/products", headers=headers, json=product_data
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_orders(self, params: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        try:
            headers = self._get_headers()
            response = await self.client.get(
                f"{self.ecommerce_base_url}/api/orders", headers=headers, params=params
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
    ...
    async def get_categories(self) -> list[dict[str, Any]]:
        ...
        try:
            headers = self._get_headers()
            response = await self.client.get(
                f"{self.ecommerce_base_url}/api/categories", headers=headers
            )
            response.raise_for_status()
            return response.json()
        except Exception as e:
...
ecommerce_service: EcommerceIntegrationService | None = None


def get_ecommerce_service() -> EcommerceIntegrationService | None:
    """Get the E-commerce integration service instance."""
    global ecommerce_service
    return ecommerce_service


def init_ecommerce_service(base_url: str, api_key: str | None = None):
