...
from .security_extended import apply_security_best_practices
from .zero_trust_security import ZeroTrustConfig, security_manager
...
def initialize_security(app: FastAPI) -> list:
    ...
    if settings.ENABLE_RATE_LIMIT:
        ...
        logger.info(
            "RateLimitMiddleware enabled (AUTH_RATE_LIMIT_MAX=%s)", settings.AUTH_RATE_LIMIT_MAX
        )
    ...
    if enable_spider:
        app.add_middleware(
            ...
            max_requests=getattr(
                settings,
                "SPIDERGUARD_MAX_REQUESTS",
                config["rate_limiting"]["default_requests_per_minute"],
            ),
            window_seconds=getattr(
                settings,
                "SPIDERGUARD_WINDOW_SECONDS",
                config["rate_limiting"]["burst_window_seconds"],
            ),
        )
    ...
    if is_production():
        ...
        logger.info(
            f"Development security settings applied (env: {config.get('environment', 'unknown')})"
        )
...
def authenticate_request(request: Request) -> bool:
    ...
    security_manager.log_access_event(client_ip, endpoint, is_authenticated, user_agent)
