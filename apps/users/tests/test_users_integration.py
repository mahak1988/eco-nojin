...
from apps.shared_core import security # Import the central security module
...
class TestUserSharedCoreIntegration:
    ...
    async def test_register_user_hashes_password_via_shared_security(self, service, mock_repo):
        ...
        created_user_mock = User(id=999, email=user_data.email, full_name=user_data.full_name, hashed_password="mock_hash")
        ...
        with patch('apps.users.service.get_password_hash') as mock_hash_func:
            ...
            call_args = mock_repo.create.call_args[0][0] # Get the first positional argument (the User object)
            assert call_args.hashed_password == "mock_hash_from_shared_core"

    @pytest.mark.asyncio
    async def test_authenticate_user_verifies_password_via_shared_security(self, service, mock_repo):
        """
