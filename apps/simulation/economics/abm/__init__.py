...
class ABMSimulator(BaseSimulator):
    @property
    def id(self) -> str: return "abm"
    @property
    def name(self) -> str: return "Agent-Based Economic Model"
    @property
    def category(self) -> str: return "economics"
    @property
    def description(self) -> str: return "Agent-based economic model for simulating individual agent behavior and market dynamics. Current skeleton implementation."
    @property
    def version(self) -> str: return "1.0.0-skeleton"

    def get_parameters(self) -> list[SimulationParameter]:
        return [
            SimulationParameter(name="num_agents", label="Number of Agents", type="int", 
                              default=100, min_value=10, max_value=1000, 
                              description="Number of economic agents in the simulation", required=True),
            SimulationParameter(name="market_type", label="Market Type", type="select", 
                              options=["perfect_competition", "monopoly", "oligopoly", "monopolistic"], 
                              default="perfect_competition", description="Type of market structure", required=True),
            SimulationParameter(name="agent_behavior", label="Agent Behavior", type="select", 
                              options=["rational", "bounded_rational", "adaptive_learning", "random"], 
                              default="rational", description="Behavioral model for agents", required=True),
            SimulationParameter(name="resource_availability", label="Resource Availability", type="float", 
                              default=1.0, min_value=0.1, max_value=5.0, 
                              description="Relative abundance of resources (1.0 = balanced)", required=True),
            SimulationParameter(name="price_volatility", label="Price Volatility", type="float", 
                              default=0.1, min_value=0.0, max_value=1.0, 
                              description="Degree of price volatility in the market", required=True),
            SimulationParameter(name="simulation_steps", label="Simulation Steps", type="int", 
                              default=50, min_value=10, max_value=200, 
                              description="Number of time steps to simulate", required=True),
        ]
...
try:
    from .wrapper import ABMWrapper, ABMOutput
...
__all__ = ["ABMWrapper", "ABMOutput", "AgentModel", "ABMSimulator"]