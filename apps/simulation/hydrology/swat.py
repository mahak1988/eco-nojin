...
class SWATSimulator(BaseSimulator):
    ...
    def _get_parameters(self) -> list[SimulationParameter]:
        ...
        return [
            ...
            SimulationParameter(
                name="et0",
                label="Reference ET0 (mm/yr)",
                type="float",
                default=600.0,
                min_value=100.0,
                max_value=2500.0,
                unit="mm",
                description="Annual reference evapotranspiration",
            ),
            SimulationParameter(
                name="runoff_coef",
                ...
                min_value=0.05,
                max_value=0.9,
                description="Fraction of precipitation becoming surface runoff",
            ),
            SimulationParameter(
                name="baseflow",
                label="Baseflow (mm/yr)",
                type="float",
                default=120.0,
                min_value=0.0,
                max_value=1000.0,
    ...
    async def _run_simulation(self, params: dict[str, Any]) -> dict:
        ...
        for t in range(n):
            ...
            flow.append(round(ro + b, 1))
            runoff.append(round(ro, 1))
            bf.append(round(b, 1))
        total = sum(flow)
        return {
            "series": [
