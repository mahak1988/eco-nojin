...
class WEAPSimulator(BaseSimulator):
    @property
    def id(self) -> str: return "weap"
    @property
    def name(self) -> str: return "WEAP Water Resources Planner"
    @property
    def category(self) -> str: return "hydrology"
    @property
    def description(self) -> str: return "Water Evaluation and Planning System for integrated water resources planning. Current skeleton implementation."
    @property
    def version(self) -> str: return "1.0.0-skeleton"

    def get_parameters(self) -> list[SimulationParameter]:
        return [
            SimulationParameter(name="watershed_area", label="Watershed Area", type="float", 
                              default=1000.0, min_value=1.0, max_value=1000000.0, unit="km²", 
                              description="Total watershed area", required=True),
            SimulationParameter(name="climate_scenario", label="Climate Scenario", type="select", 
                              options=["historical", "wet_future", "dry_future", "variable"], 
                              default="historical", description="Climate scenario to simulate", required=True),
            SimulationParameter(name="demand_category", label="Demand Category", type="select", 
                              options=["agriculture", "municipal", "industrial", "environmental", "mixed"], 
                              default="mixed", description="Primary water demand category", required=True),
            SimulationParameter(name="supply_infrastructure", label="Supply Infrastructure", type="select", 
                              options=["natural", "developed", "managed", "stress_tested"], 
                              default="developed", description="Level of supply infrastructure", required=True),
            SimulationParameter(name="population_growth", label="Population Growth", type="float", 
                              default=0.02, min_value=0.0, max_value=0.05, 
                              description="Annual population growth rate", required=True),
            SimulationParameter(name="economic_growth", label="Economic Growth", type="float", 
                              default=0.03, min_value=0.0, max_value=0.1, 
                              description="Annual economic growth rate", required=True),
            SimulationParameter(name="conservation_effort", label="Conservation Effort", type="float", 
                              default=0.1, min_value=0.0, max_value=0.5, 
                              description="Fraction of demand reduced through conservation", required=True),
        ]
    ...
    def _run_skeleton_simulation(self, params: dict[str, Any]) -> dict:
        ...
        base_supply = (base_precip * climate_multiplier * watershed_area * 1000) / 1e6  # Convert to MCM
        ...
        for year in range(n_years):
            ...
            outflow = min(total_demand, current_storage + inflow)  # Can't release more than available
        ...
        env_flow_satisfied = sum(1 for i in range(len(supplies)) 
                                if supplies[i] > environmental_flow_requirement) / len(supplies)
        
        return {
            "series": [
                {"key": "supply", "label": "Water Supply (MCM)", "color": "#3b82f6", 
                 "values": supplies, "kind": "line", "fill": True},
                {"key": "demand", "label": "Water Demand (MCM)", "color": "#ef4444", 
                 "values": demands, "kind": "line", "fill": True},
                {"key": "storage", "label": "System Storage (MCM)", "color": "#8b5cf6", 
                 "values": storages, "kind": "area", "fill": True},
                {"key": "deficit", "label": "Water Deficit (MCM)", "color": "#f97316", 
                 "values": deficits, "kind": "column", "fill": True},
            ],
            "metrics": {
