...
def run_icbm(params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    r_e = float(p.get("r_e")) if p.get("r_e") is not None else (
        _temp_factor_q10(temp, 10.0, 2.0) * _moisture_factor(rain, et)
    )
...
def run_yasso_lite(params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    k_a, k_w, k_e, k_n, k_h = 0.6 * ft * fw, 0.4 * ft * fw, 0.3 * ft * fw, 0.1 * ft * fw, 0.002 * ft * fw
