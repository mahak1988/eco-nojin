...
class RothCSimulator(BaseSimulator):
    ...
    def _get_parameters(self) -> list[SimulationParameter]:
        ...
        return [
            SimulationParameter(name="lat", label="Latitude", type="float", default=35.7, min_value=-90.0, max_value=90.0, description="Field latitude (triggers NASA climate fetch)"),
            SimulationParameter(name="lon", label="Longitude", type="float", default=51.4, min_value=-180.0, max_value=180.0, description="Field longitude"),
            SimulationParameter(name="start_date", label="Start Date", type="string", default="2020-01-01", description="Simulation start date (YYYY-MM-DD)"),
            SimulationParameter(name="end_date", label="End Date", type="string", default="2024-12-31", description="Simulation end date (YYYY-MM-DD)"),
            ...
            SimulationParameter(name="temperature", label="Mean Annual Temp (C) [Fallback]", type="float", default=15.0, min_value=-5.0, max_value=35.0, unit="C", description="Mean annual temperature (used if no lat/lon)"),
            SimulationParameter(name="moisture", label="Moisture Factor [Fallback]", type="float", default=0.8, min_value=0.1, max_value=1.5, description="Soil moisture rate modifier (used if no lat/lon)"),
            SimulationParameter(name="years", label="Simulation Years [Fallback]", type="int", default=5, min_value=1, max_value=200, unit="yr", description="Number of years to simulate (used if no dates)")
        ]
        ...
        if lat is not None and lon is not None and start_date and end_date:
            try:
                ...
                if climate_series:
                    ...
                    temps = [(float(d.get('temp_max_c', 15)) + float(d.get('temp_min_c', 15))) / 2.0 for d in climate_series.values()]
                ...
                import logging
                logging.getLogger(__name__).warning(f"NASA climate fetch failed, falling back to defaults: {e}")
