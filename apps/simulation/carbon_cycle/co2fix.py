...
class CO2FIXSimulator(BaseSimulator):
    ...
    @property
    def description(self) -> str: return "CO2FIX model for carbon stock and sequestration assessment. Current skeleton implementation."
    @property
    def version(self) -> str: return "1.0.0-skeleton"

    def get_parameters(self) -> list[SimulationParameter]:
        return [
            SimulationParameter(name="land_use", label="Land Use Type", type="select", 
                              options=["forest", "grassland", "cropland", "wetland"], 
                              default="forest", description="Type of land use", required=True),
            SimulationParameter(name="area_ha", label="Area", type="float", 
                              default=100.0, min_value=0.1, max_value=10000.0, 
                              unit="ha", description="Area in hectares", required=True),
            SimulationParameter(name="management_practice", label="Management Practice", 
                              type="select", options=["conventional", "conservation", "organic"], 
                              default="conventional", description="Management practice", required=True),
            SimulationParameter(name="years", label="Simulation Years", 
                              type="int", default=10, min_value=1, max_value=50, 
                              unit="years", description="Number of years to simulate", required=True),
            SimulationParameter(name="initial_carbon_stock", label="Initial Carbon Stock", 
                              type="float", default=50.0, min_value=0.0, max_value=500.0, 
                              unit="tC/ha", description="Initial carbon stock", required=True),
        ]
        ...
        return {
            "series": [
                {"key": "carbon_stock", "label": "Carbon Stock (tC/ha)", "color": "#10b981", 
                 "values": monthly_values, "kind": "line", "fill": True},
            ],
            "metrics": {
