...
class WOFOSTSimulator(BaseSimulator):
    ...
    @property
    def name(self) -> str: return "WOFOST Crop Growth Simulation Model"
    @property
    def category(self) -> str: return "agriculture"
    @property
    def description(self) -> str: return "WOFOST model for crop growth simulation and production quantification. Current skeleton implementation."
    @property
    def version(self) -> str: return "1.0.0-skeleton"

    def get_parameters(self) -> list[SimulationParameter]:
        return [
            SimulationParameter(name="crop", label="Crop Type", type="select", 
            ...
            SimulationParameter(name="planting_date", label="Planting Date", type="string", 
                              default="2024-04-01", description="Planting date (YYYY-MM-DD)", required=True),
            SimulationParameter(name="latitude", label="Latitude", type="float", 
                              default=52.1, min_value=-90.0, max_value=90.0, unit="deg", 
                              description="Latitude of location", required=True),
            SimulationParameter(name="longitude", label="Longitude", type="float", 
                              default=5.18, min_value=-180.0, max_value=180.0, unit="deg", 
                              description="Longitude of location", required=True),
            SimulationParameter(name="soil_type", label="Soil Type", type="select", 
                              options=["clay", "loam", "sand", "peat"], default="loam", 
                              description="Soil type", required=True),
            SimulationParameter(name="irrigation", label="Irrigation Level", type="select", 
                              options=["rainfed", "limited", "full"], default="rainfed", 
                              description="Irrigation level", required=True),
        ]
    ...
    def _run_skeleton_simulation(self, params: dict[str, Any]) -> dict:
        ...
        return {
            "series": [
                ...
                {"key": "lai", "label": "Leaf Area Index", "color": "#22c55e", 
                 "values": lai_values, "kind": "line", "fill": False},
            ],
            "metrics": {
