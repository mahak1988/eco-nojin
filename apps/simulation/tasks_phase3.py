"""Celery tasks — AquaCrop advanced + SWAT+ proxy with run persistence."""
...
def task_aquacrop_advanced(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning("persist aquacrop failed: %s", e)
        result["run_id"] = None
...
def task_swat(self, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    task_id = getattr(self.request, "id", None)
    try:
        rid = save_run_sync("swat_plus_proxy", params, result, task_id=task_id, farm_id=farm_id)
        result["run_id"] = rid
    except Exception as e:
        logger.warning("persist swat failed: %s", e)
        result["run_id"] = None
        result["persist_error"] = str(e)[:120]
    result["task_id"] = task_id
    result["mode"] = "celery"
