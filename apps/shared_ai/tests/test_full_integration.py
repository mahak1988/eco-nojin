...
async def test_full_integration():
    ...
    login_resp = client.post(
        "/api/users/login", json={"email": "full_test@example.com", "password": "securepass123"}
    )
    ...
    if analyst_resp.status_code == 200:
        ...
        logger.info("✅ Data Analyst responded")
        logger.info(f"📝 Preview: {data['assistant_message'][:200]}...")
    else:
        logger.error("❌ Data Analyst test failed")
    ...
    if diff_resp.status_code == 200:
        data = diff_resp.json()
        logger.info("✅ Differential equation solved")
        logger.info(f"📝 Preview: {data['assistant_message'][:200]}...")
    else:
        logger.error("❌ Differential equation test failed")
    ...
    if ml_resp.status_code == 200:
        ...
        logger.error("❌ ML training test failed")
    ...
    if fallback_resp.status_code == 200:
        ...
        logger.info("✅ Fallback system worked")
        logger.info(f"📝 Preview: {data['assistant_message'][:200]}...")
    else:
