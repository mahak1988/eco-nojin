...
async def test_rag():
    ...
    result1 = await upload_document.ainvoke(
        {"content": doc1, "title": "مقدمه‌ای بر هوش مصنوعی", "user_id": user_id, "file_type": "txt"}
    )
    logger.info(f"✅ Document 1 uploaded:\n{result1}")

    result2 = await upload_document.ainvoke(
        {"content": doc2, "title": "پایگاه‌های داده برداری", "user_id": user_id, "file_type": "txt"}
    )
    ...
    search_result = await search_knowledge_base.ainvoke(
        {"query": "یادگیری ماشین چیست؟", "user_id": user_id, "top_k": 3}
    )
    ...
    context = await get_rag_context.ainvoke(
        {"query": "Qdrant و FAISS چه تفاوتی دارند؟", "user_id": user_id, "top_k": 2}
    )
