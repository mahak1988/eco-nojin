...
async def test_fallback_system():
    ...
    login_resp = client.post(
        "/api/users/login", json={"email": "fallback_test@example.com", "password": "securepass123"}
    )
    ...
    admin_resp = client.post(
        "/api/ai-agents/chat",
        headers=headers,
        json={"agent_type": "admin", "message": "متدولوژی‌های مدیریت پروژه را توضیح بده"},
    )
    ...
    code_resp = client.post(
        "/api/ai-agents/chat",
        headers=headers,
        json={"agent_type": "code_assistant", "message": "اصول کدنویسی تمیز را توضیح بده"},
    )
