...
async def test_streaming():
    ...
    with client.stream(
        ...
        headers=headers,
        json={"agent_type": "support", "message": "سلام! چگونه رمز عبور را تغییر دهم؟"},
    ) as response:
                ...
                try:
                    ...
                    if "conversation_id" in data:
                        ...
                        if chunk_count == 1:
                            ...
                            logger.info(
                                f"📝 Chunk #{chunk_count} received ({len(full_content)} chars total)"
                            )
    ...
    if chunk_count > 0:
        logger.info("✅ Support Agent streaming test PASSED")
    else:
        logger.error("❌ Support Agent streaming test FAILED")
        return
    ...
    with client.stream(
        ...
        "/api/ai-agents/chat/stream",
        headers=headers,
        json={"agent_type": "financial", "message": "سلام! نسبت‌های مالی مهم را توضیح بده"},
    ) as response:
        logger.info("✅ Financial streaming started")
            ...
            if line.startswith("data: "):
                ...
                try:
                    ...
                    if "content" in data:
                        ...
                        used_fallback = data.get("used_fallback", False)
                        logger.info("✅ Financial stream completed")
    ...
    if chunk_count > 0:
        logger.info("✅ Financial Agent streaming test PASSED")
    else:
        logger.error("❌ Financial Agent streaming test FAILED")
