...
from apps.shared_core.database.session import Base, async_session_maker, close_db, init_db
...
async def test_integration():
    ...
    async with async_session_maker() as session:
        ...
        await product_repo.create(
            {"name": "لپ‌تاپ ایسوس", "price": 45000000, "category": "الکترونیک"}
        )
        await product_repo.create(
            {"name": "گوشی سامسونگ", "price": 25000000, "category": "الکترونیک"}
        )
    ...
    fake_llm = FakeToolCallingModel(
        responses=[
                ...
                tool_calls=[
                    {
                        "name": "query_database",
                        "args": {
                            "sql_query": "SELECT name, price FROM products WHERE category = 'الکترونیک'"
                        },
                        "id": "1",
                    }
                ],
            ...
            AIMessage(
                content="بر اساس کوئری دیتابیس، دو محصول الکترونیکی یافت شد: لپ‌تاپ ایسوس با قیمت ۴۵ میلیون تومان و گوشی سامسونگ با قیمت ۲۵ میلیون تومان."
            ),
        ]
    )
