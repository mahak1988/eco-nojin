...
async def test_fast_compute():
    ...
    try:
        ...
        result = await monte_carlo_simulation.ainvoke(
            {"iterations": 10000, "steps": 100, "up_factor": 1.1, "down_factor": 0.9}
        )
        logger.info(f"✅ Result:\n{result[:300]}...")
    except Exception as e:
    ...
    try:
        ...
        result = await solve_differential_equation.ainvoke(
            {"initial_conditions": [1.0], "time_span": [0.0, 5.0], "coefficients": [0.5]}
        )
        logger.info(f"✅ Result:\n{result[:300]}...")
    except Exception as e:
    ...
    try:
        ...
        result = await advanced_matrix_operations.ainvoke({"matrix": matrix, "operation": "eigen"})
        logger.info(f"✅ Result:\n{result[:300]}...")
    except Exception as e:
    ...
    try:
        ...
        result = await train_ml_model.ainvoke({"X": X, "y": y, "model_type": "linear_regression"})
        logger.info(f"✅ Result:\n{result[:300]}...")
    except Exception as e:
    ...
    try:
        ...
        result = await numerical_integration.ainvoke(
            {"coefficients": [0.0, 0.0, 1.0], "bounds": [0.0, 1.0]}
        )
        logger.info(f"✅ Result:\n{result[:300]}...")
    except Exception as e:
