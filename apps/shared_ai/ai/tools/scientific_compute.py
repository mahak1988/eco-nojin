...
async def solve_differential_equation(
    ...
    try:
        ...
        output.extend(["", "⚡ Performance: SciPy solve_ivp"])

        return "\n".join(output)

    except Exception as e:
        logger.error(f"❌ Differential equation error: {e}")
        return f"❌ خطا در حل معادله: {e!s}"
...
@tool
async def advanced_matrix_operations(matrix: list[float], operation: str = "eigen") -> str:
    """
    ...
    try:
        ...
        elapsed = (time.time() - start_time) * 1000
        output.extend(
            ["", f"⏱️ زمان محاسبه: {elapsed:.2f} ms", "⚡ Performance: SciPy linalg (LAPACK)"]
        )
        ...
        return f"❌ خطا در عملیات ماتریسی: {e!s}"
...
async def train_ml_model(
    X: list[float], y: list[float], model_type: str = "linear_regression", degree: int = 2
) -> str:
    """
    ...
    try:
        ...
        if model_type == "linear_regression":
            ...
            output = [
                "🤖 نتایج آموزش مدل (linear_regression):",
                "",
                "📊 ضرایب مدل:",
        ...
        elapsed = (time.time() - start_time) * 1000
        output.extend(["", f"⏱️ زمان محاسبه: {elapsed:.2f} ms", "⚡ Performance: NumPy/SciPy lstsq"])

        return "\n".join(output)

    except Exception as e:
        logger.error(f"❌ ML training error: {e}")
        return f"❌ خطا در آموزش مدل: {e!s}"
...
@tool
async def numerical_integration(coefficients: list[float], bounds: list[float]) -> str:
    """
    ...
    try:
        ...
        output = [
            ...
            "",
            "⚡ Performance: SciPy quad (Gaussian quadrature)",
        ]
        ...
        return f"❌ خطا در انتگرال‌گیری: {e!s}"
...
async def scientific_optimization(
    ...
    try:
        ...
        return f"❌ خطا در بهینه‌سازی: {e!s}"
