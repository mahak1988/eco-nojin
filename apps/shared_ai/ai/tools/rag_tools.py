...
async def upload_document(
    ...
    try:
        ...
        output = [
            ...
            "   • نوع chunking: Semantic",
            f"   • Document IDs: {len(doc_ids)}",
            "",
            "💡 سند اکنون در جستجوی Hybrid (BM25 + Vector) در دسترس است.",
        ]
        ...
        return f"❌ خطا در آپلود سند: {e!s}"


@tool
async def search_knowledge_base(
    query: str, user_id: int, top_k: int = 5, use_reranking: bool = True
) -> str:
    """
    ...
    try:
        ...
        output = [
            "🔍 نتایج جستجو در پایگاه دانش (Hybrid + Reranking):",
        ...
        return f"❌ خطا در جستجو: {e!s}"


@tool
async def get_rag_context(
    query: str, user_id: int, top_k: int = 3, use_reranking: bool = True
) -> str:
    """
...
@tool
async def delete_document(title: str, user_id: int) -> str:
    ...
    try:
        ...
        results = hybrid_search.search(
            query=title, top_k=100, filter_metadata={"user_id": user_id, "title": title}
        )
        ...
        hybrid_search.documents = [
            doc for doc in hybrid_search.documents if doc not in texts_to_remove
        ]
...
async def get_knowledge_base_stats(user_id: int) -> str:
    ...
    try:
        ...
        return f"❌ خطا در دریافت آمار: {e!s}"
