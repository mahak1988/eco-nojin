    ...
    try:
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                if node.end_lineno and (node.end_lineno - node.lineno) > 50:
                    issues.append(
                        f"⚠️ تابع '{node.name}' در خط {node.lineno} بیش از 50 خط است (توصیه: تقسیم به توابع کوچکتر)"
                    )
        ...
        if functions:
            ...
            for func in functions:
                ...
                output.append(
                    f"   - {async_mark}{func['name']}({', '.join(func['args'])}) [خط {func['line']}, {func['lines_count']} خط] docstring:{doc_mark}"
                )
...
def _find_bugs_extracted():
    ...
    try:
        ...
        return f"❌ خطا در شناسایی باگ: {e!s}"
...
async def calculate_complexity(code: str, function_name: str | None = None) -> str:
    ...
    try:
        ...
        if not results:
            return (
                f"❌ تابعی با نام '{function_name}' یافت نشد"
                if function_name
                else "❌ هیچ تابعی یافت نشد"
            )
        ...
        for result in results:
            if result["time_complexity"] in ["n²", "n³", "2ⁿ"]:
                output.append(
                    f"   - تابع '{result['name']}' پیچیدگی زمانی بالایی دارد. بهینه‌سازی پیشنهاد می‌شود."
                )
            if result["time_complexity"] == "n²":
                output.append(
                    f"   - برای '{result['name']}' از الگوریتم‌های O(n log n) استفاده کنید."
                )
        ...
        return f"❌ خطا در محاسبه پیچیدگی: {e!s}"
...
def _analyze_complexity(func_node: ast.FunctionDef) -> dict[str, Any]:
    ...
    return {"time": time_complexity, "space": space_complexity, "details": details}
...
def _generate_pytest(func_name: str, args: list[str], defaults: list, returns) -> str:
    ...
    if args:
        ...
        empty_args = ", ".join(
            [f'{arg}=""' if i == 0 else f"{arg}=None" for i, arg in enumerate(args)]
        )
...
async def generate_documentation(code: str, style: str = "google") -> str:
    ...
    try:
        ...
        return f"❌ خطا در تولید مستندات: {e!s}"
