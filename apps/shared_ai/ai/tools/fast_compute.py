...
async def fast_statistics(data: list[float], operations: list[str] | None = None) -> str:
    ...
    try:
        ...
        return f"❌ خطا در محاسبات آماری: {e!s}"
...
@jit(nopython=True, parallel=True, cache=True)
def _monte_carlo_random_walk(
    iterations: int, steps: int, up_factor: float, down_factor: float, seed: int
) -> None:
...
def _calculate_confidence_interval(
    data: np.ndarray, mean: float, std: float, confidence: float = 0.95
) -> None:
    """محاسبه بازه اطمینان."""
    n = len(data)
...
async def monte_carlo_simulation(
    ...
    try:
        ...
        results = _monte_carlo_random_walk(iterations, steps, up_factor, down_factor, seed=42)
...
async def optimization_solver(
    ...
    try:
        ...
        # حل بهینه‌سازی
        result = minimize(
            objective, x0, method="L-BFGS-B", bounds=bounds, options={"maxiter": 1000, "ftol": 1e-9}
        )
