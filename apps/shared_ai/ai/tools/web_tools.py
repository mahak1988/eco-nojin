...
async def summarize_text(text: str, max_length: int = 500) -> str:
    ...
    try:
        ...
        return f"❌ خطا در خلاصه‌سازی: {e!s}"
...
async def extract_key_points(text: str, num_points: int = 5) -> str:
    ...
    try:
        ...
        return f"❌ خطا در استخراج نکات کلیدی: {e!s}"
...
async def fetch_url_content(url: str) -> str:
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                url, headers={"User-Agent": "Mozilla/5.0 (Research Agent)"}, timeout=15.0
            )
        ...
        return f"❌ خطا در دریافت محتوا: {e!s}"
