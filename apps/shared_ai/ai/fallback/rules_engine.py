...
class RulesEngine:
    ...
    async def evaluate(self, agent_type: str, context: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for rule in rules:
            if self._check_condition(rule.condition, context):
                matched_rules.append(
                    {"rule_name": rule.rule_name, "action": rule.action, "priority": rule.priority}
                )
    ...
    async def get_actions_summary(self, agent_type: str, context: dict[str, Any]) -> str:
