...
class VectorStore:
    ...
    def _initialize(self) -> None:
        ...
        if qdrant_url and qdrant_api_key:
            try:
                ...
                self._client = QdrantClient(url=qdrant_url, api_key=qdrant_api_key, timeout=10.0)
        ...
        # تلاش برای Qdrant Local
            ...
            try:
                ...
                self._client = QdrantClient(host=qdrant_host, port=qdrant_port, timeout=10.0)
    ...
    def add_documents(self, texts: list[str], metadata_list: list[dict[str, Any]]) -> list[str]:
        """
        ...
        if self._backend == "qdrant":
            ...
            points = [
                models.PointStruct(id=doc_id, vector=embedding, payload={"text": text, **metadata})
                for doc_id, embedding, text, metadata in zip(
                    doc_ids, embeddings, texts, metadata_list
                )
            ]
            ...
            for doc_id, text, metadata in zip(doc_ids, texts, metadata_list):
                self._faiss_data.append({"id": doc_id, "text": text, **metadata})
    ...
    def delete_documents(self, doc_ids: list[str]) -> bool:
        ...
        if self._backend == "qdrant":
            self._client.delete(collection_name=self.collection_name, points_selector=doc_ids)
    ...
    def _rebuild_faiss(self, exclude_ids: set) -> None:
        ...
        # فیلتر کردن داده‌ها
        filtered_data = [doc for doc in self._faiss_data if doc["id"] not in exclude_ids]
