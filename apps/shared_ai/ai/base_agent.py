...
class ModularAgentBuilder:
    ...
    async def _call_tool(self, state: AgentState) -> AgentState:
        ...
        for tc in tool_calls:
            ...
            try:
                ...
                tool_results.append(
                    ToolMessage(content=f"❌ خطا در اجرای ابزار: {e!s}", tool_call_id=tc["id"])
                )
    ...
    async def run(self, user_input: str, context: dict = None) -> str:
        ...
        initial_state = {"messages": [HumanMessage(content=user_input)], "context": context or {}}
    ...
    async def run_stream(self, user_input: str, context: dict = None) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async for event in self.graph.astream_events(
                initial_state, config={"recursion_limit": 25}, version="v2"
            ):
                kind = event["event"]
