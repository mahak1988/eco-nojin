...
class LLMFactory:
    ...
    def _create_groq(model: str, temperature: float, **kwargs) -> Any:
        ...
        return ChatGroq(model=model, api_key=api_key, temperature=temperature, **kwargs)
    ...
    def _create_ollama(model: str, temperature: float, **kwargs) -> Any:
        ...
        return ChatOllama(
            model=model, base_url=base_url, temperature=temperature, timeout=120, **kwargs
        )
    ...
    def _create_fake(model: str, temperature: float, **kwargs) -> Any:
        ...
        return FakeToolCallingModel(
            responses=[
                AIMessage(
                    content="این یک پاسخ آزمایشی است. لطفاً یکی از API Keyها را در فایل .env تنظیم کنید:"
                )
            ]
        )
