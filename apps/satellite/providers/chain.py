...
class ProviderChain:
    ...
    async def by_role(
        self, role: str, lat: float, lon: float, date: str | None = None
    ) -> dict[str, Any]:
        if role == "topography":
            topo = next(
                (p for p in self.providers if isinstance(p, OpenTopoProvider)), OpenTopoProvider()
            )
            return await topo.elevation(lat, lon)
        if role == "thermal":
            th = next(
                (p for p in self.providers if isinstance(p, ThermalProvider)), ThermalProvider()
            )
