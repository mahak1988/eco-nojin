...
interface RealtimeEvent {
  event_type: string;
  timestamp: number; // ثانیه از epoch
  message: string;
}
...
function App() {
  ...
  const fetchRegions = async () => {
    try {
      ...
      // فرض: backend چیزی مثل { regions: [...] } برمی‌گرداند
  ...
  const fetchAnalyses = async () => {
    try {
      ...
      const data = await res.json();
      // فرض: backend چیزی مثل { analyses: [...] } برمی‌گرداند
                ...
                >
                  <Popup>
                    <div className="popup-content">
                      ...
                      <br />
                      سود:{" "}
                      {(analysis.profit / 1_000_000).toFixed(2)}{" "}
                      میلیون تومان
                    </div>
                  </Popup>
                </Circle>
