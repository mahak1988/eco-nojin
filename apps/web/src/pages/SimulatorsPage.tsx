// API-driven simulators list; UI strings + API lang follow useLang (fa/en/ar).
...
export default function SimulatorsPage() {
  const { lang } = useLang();
  const simLang = (lang === "fa" || lang === "ar" || lang === "en" ? lang : "en") as SimLang;
  const s = SIM_STR[simLang];
  const locale = localeOf(simLang);
  ...
  const [states, setStates] = useState<Record<string, SimState>>(() =>
    Object.fromEntries(
      SIM_CONFIGS.map((c) => [
        c.id,
        { params: defaultParams(c.id), progress: 0, status: "idle" as const, runs: 0 },
      ]),
    ),
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      setBackend("loading");
      const sims = await fetchSimulators(simLang);
      if (cancelled) return;
      if (!sims || sims.length === 0) {
        setBackend("offline");
        setConfigs(SIM_CONFIGS);
        setParamDefs(PARAM_DEFS);
        return;
      }
      const paramResults = await Promise.all(sims.map((sim) => fetchParameters(sim.id, simLang)));
      if (cancelled) return;
          ...
          if (next[k].status === "running") {
            ...
            if (np !== next[k].progress) {
              ...
              next[k] = {
                ...
                runs: done ? next[k].runs + 1 : next[k].runs,
              };
            }
          }
        }
  ...
  const run = (id: string) =>
    setStates((p) => ({
      ...
      [id]: {
        ...p[id],
        status: "running",
        progress: p[id].status === "done" ? 0 : p[id].progress,
      },
    }));
  const stop = (id: string) => setStates((p) => ({ ...p, [id]: { ...p[id], status: "idle" } }));
  const reset = (id: string) =>
    setStates((p) => ({
      ...p,
      [id]: {
        params: Object.fromEntries((paramDefs[id] ?? []).map((d) => [d.key, d.default])),
        progress: 0,
        status: "idle",
        runs: p[id].runs,
      },
  ...
  const filtered = useMemo(() => {
    ...
    return configs.filter(
      (c) =>
        ...
        (q === "" || simText(s, c.nameKey).toLowerCase().includes(q) || c.nameKey.toLowerCase().includes(q)),
    );
  }, [search, filter, states, configs, s]);
  ...
  const backendLabel =
    backend === "online"
      ? `${configs.length} ${s.backend_online}`
      : backend === "offline"
        ? s.backend_offline
        : s.backend_loading;
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-5 sm:p-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold ring-1 ${
                backend === "online"
                  ...
                  : backend === "offline"
                    ? "bg-stone-100 text-stone-600 ring-stone-600/15"
            ...
            <button
              type="button"
              onClick={exportAll}
              className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold shadow-sm transition-all hover:-translate-y-0.5 ${
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative min-w-[200px] flex-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-stone-200 bg-white py-2.5 pe-3 ps-9 text-sm text-stone-800 outline-none transition-colors placeholder:text-stone-400 focus:border-green-500 focus:ring-2 focus:ring-green-500/15"
            />
          </div>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-stone-300 bg-white py-16 text-center">
          ...
          <p className="text-stone-500">{s.filterAll}</p>
        </div>
      ) : (
