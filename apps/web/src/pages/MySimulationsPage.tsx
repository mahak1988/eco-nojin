...
interface SavedRun {
  ...
  advisory?: { recommendations?: { text: string }[] };
...
export default function MySimulationsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await fetch(url(`${API_V1}/simulation/runs?limit=100`), { credentials: "include" });
      if (r.ok) {
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-5 sm:p-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-stone-300 bg-white py-16 text-center">
          ...
          <p className="text-stone-500">No saved simulations yet.</p>
          <Link
            to="/simulators"
            ...
            <div key={run.id} className="rounded-2xl border border-stone-200 bg-white p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <Link
                      to={`/simulators/${run.simulator_id}`}
                      className="font-display text-lg text-stone-800 hover:text-green-700"
                    >
                ...
                <button
                  ...
                  className="grid h-9 w-9 place-items-center rounded-xl border border-stone-200 text-stone-500 hover:bg-red-50 hover:text-red-600"
                >
