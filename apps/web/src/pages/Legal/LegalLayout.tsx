 ...
 * 
 * Features:
 * - Responsive table of contents (sidebar on desktop, collapsible on mobile)
    ...
    <div dir={dir} className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-emerald-50/30">
      ...
      <header className="relative overflow-hidden bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900 text-white">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-16 sm:py-20 lg:px-8">
          <Link
            to="/"
            className="mb-8 inline-flex items-center gap-2 text-sm text-emerald-100 transition hover:text-white"
          >
          ...
          {subtitle && (
            <p className="mt-4 max-w-2xl text-lg text-emerald-100 leading-relaxed">
              {subtitle}
            </p>
          )}
          <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-emerald-200">
            ...
            <span className="inline-flex items-center gap-2">
              ...
              {t("legal.compliantWith")}: GDPR, ISO 27001, UN SDGs
            </span>
          </div>
        </div>
      </header>
      ...
      {/* Mobile TOC Toggle */}
      <div className="sticky top-0 z-40 border-b border-gray-200 bg-white/90 backdrop-blur-md lg:hidden">
        <button
        ...
        >
          ...
          <svg
            className={cn("h-5 w-5 transition-transform", isTocOpen && "rotate-180")}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        {isTocOpen && (
          <nav className="border-t border-gray-100 bg-white px-6 py-4">
              ...
              {toc.map((item) => (
                <li key={item.id}>
                  <button
                    ...
                    className={cn(
                      "w-full text-start text-sm transition",
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              ...
              <nav className="space-y-1 border-s-2 border-gray-100 ps-4">
                {toc.map((item) => (
                  <button
                    ...
                    onClick={() => scrollToSection(item.id)}
                    className={cn(
                      "block w-full text-start text-sm transition-all",
                      activeSection === item.id
                        ? "border-s-2 -ms-[18px] border-emerald-500 ps-4 font-semibold text-emerald-700"
                        : "text-gray-600 hover:text-emerald-600"
                    )}
                  >
                    {item.title}
              ...
              <button
                ...
                className="mt-8 inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:bg-gray-50"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export function LegalCallout({
  ...
  type?: "info" | "warning" | "success" | "principle";
  title: string;
  children: ReactNode;
}): JSX.Element {
  const styles = {
    ...
    principle: "bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-300 text-emerald-900",
  };
