...
import { shortAddr, type EcoStrings, type EcoLang } from "./ecocoinI18n";
import type { WalletAction } from "./WalletActionsModal";
...
export function WalletCard({ address, balance, staked, apy, strings: s, lang, onAction }: Props) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-3xl p-6 text-white shadow-xl sm:p-7"
    ...
    >
      ...
      <div className="relative mt-5 flex items-center gap-4 rounded-2xl bg-white/10 p-3 backdrop-blur">
        ...
        <div className="flex-1">
          ...
          <p className="font-bold tabular-nums">
            {staked.toLocaleString(locale)}{" "}
            <span className="text-emerald-200/80">{s.ecoUnit}</span>
          </p>
        </div>
      ...
      <div className="relative mt-5 grid grid-cols-3 gap-2">
        <button
          ...
          className="flex flex-col items-center gap-1 rounded-xl bg-white/10 py-2.5 text-xs font-bold backdrop-blur transition-colors hover:bg-white/20 active:scale-95"
        >
        ...
        <button
          ...
          className="flex flex-col items-center gap-1 rounded-xl bg-emerald-400 py-2.5 text-xs font-bold text-emerald-950 transition-colors hover:bg-emerald-300 active:scale-95"
        >
