...
// روش‌های پرداخت — آیکون‌ها CSS/emoji. تعامل: set-default + افزودن/حذف کارت.
import { useState } from "react";
import { CreditCard, Star, Plus, Check, X, Trash2 } from "lucide-react";
...
export function PaymentMethods({ methods, strings: s, lang, onSetDefault, onAddCard, onRemoveCard, canManage = true }: Props) {
  ...
  const sub = (m: PaymentMethod): string => {
    if (m.kind === "credit_card") return `•••• ${m.last4}${m.holder ? ` · ${m.holder}` : ""}`;
    ...
    <SectionReveal delay={80}>
      <div className="rounded-2xl border border-stone-200/80 bg-white p-5 shadow-sm sm:p-6">
        <div className="mb-4 flex items-center justify-between">
          ...
          {canManage && !adding && (
            <button type="button" onClick={() => setAdding(true)} className="inline-flex items-center gap-1.5 rounded-xl border border-stone-200 px-3 py-1.5 text-xs font-bold text-stone-700 transition-colors hover:bg-stone-50">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {methods.map((m) => {
            ...
            return (
              <div key={m.id} className={`relative rounded-xl border p-4 text-center transition-all ${m.isDefault ? "border-green-300 bg-green-50/40 ring-1 ring-green-600/10" : "border-stone-200 hover:border-stone-300"}`}>
                {m.isDefault && (
                ...
                <p className="font-semibold text-stone-800">{payText(s, `method_${m.kind}` as keyof PaymentStrings)}</p>
                <p className="mt-0.5 text-xs text-stone-500">{sub(m)}</p>
                <div className="mt-2 flex flex-wrap items-center justify-center gap-2">
                  {fiat && !m.isDefault && canManage && (
                    <button type="button" onClick={() => onSetDefault(m.id)} className="text-xs font-bold text-green-700 hover:underline">{s.setDefault}</button>
                  )}
                  {!fiat && <p className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700"><Check className="h-3 w-3" />{s.connected}</p>}
                  {fiat && canManage && onRemoveCard && (
                    <button type="button" onClick={() => onRemoveCard(m.id)} className="inline-flex items-center gap-0.5 text-xs font-bold text-red-600 hover:underline" title={s.removeCard}>
                      <Trash2 className="h-3 w-3" />{s.removeCard}
                    </button>
                  )}
                </div>
              </div>
        ...
        {adding && canManage && (
          <div className="mt-4 rounded-xl border border-stone-200 bg-stone-50 p-4" style={{ animation: "fade-up .25s ease-out" }}>
            <p className="mb-3 text-sm font-semibold text-stone-700">{s.addCard}</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={lang === "fa" ? "نام روی کارت" : "Name on card"}
                  className="w-full rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-800 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/15"
                />
              </div>
            </div>
            ...
            <div className="mt-3 flex items-center gap-2">
              <button type="button" onClick={submit} className="rounded-xl bg-green-600 px-4 py-2 text-sm font-bold text-white shadow-sm transition-colors hover:bg-green-700">{s.add}</button>
              <button type="button" onClick={() => { setAdding(false); setErr(""); setLast4(""); setHolder(""); }} className="inline-flex items-center gap-1 rounded-xl border border-stone-200 bg-white px-4 py-2 text-sm font-bold text-stone-700 hover:bg-stone-50">
                <X className="h-3.5 w-3.5" />{s.cancel}
              </button>
            </div>
          </div>
        )}
