...
// table دسکتاپ + card موبایل + expandable + وضعیت/حذف
import { useState } from "react";
import {
  CheckCircle2, Clock, XCircle, ChevronDown, Eye, CreditCard,
  ArrowUp, ArrowDown, ArrowUpDown, Trash2, Check, Ban,
} from "lucide-react";
import type { Payment, PaymentStatus, SortKey, SortDir } from "./paymentsData";
import { formatFullDate, shortRef } from "./paymentsData";
import { statusText, methodText, formatAmount, localeOf, type PaymentStrings, type PayLang } from "./paymentsI18n";
...
function amountDisplay(p: Payment, lang: PayLang): string {
  if (p.method === "credit_card" || p.method === "bank_transfer") {
    const cs = readCurrencySettings();
    const locale = lang === "fa" ? "fa-IR" : lang === "ar" ? "ar-EG" : "en-US";
    return formatMoney(p.amount, cs.primary, cs, locale);
  }
  return formatAmount(p.method, p.amount, lang);
}

export function PaymentsTable({ payments, strings: s, lang, sortKey, sortDir, onSort, onSetStatus, onDelete }: Props) {
  ...
  const Detail = ({ p }: { p: Payment }) => (
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          <p className="text-[11px] font-bold uppercase text-stone-400">{s.detailRef}</p>
          <p className="mt-0.5 break-all font-mono text-xs text-stone-700">{p.reference}</p>
        </div>
        <div>
          <p className="text-[11px] font-bold uppercase text-stone-400">{s.detailDate}</p>
          <p className="mt-0.5 text-sm text-stone-700">{formatFullDate(p.date, locale)}</p>
        </div>
        <div>
          <p className="text-[11px] font-bold uppercase text-stone-400">{s.detailAmount}</p>
          <p className="mt-0.5 font-display text-base font-black tabular-nums text-stone-800">{amountDisplay(p, lang)}</p>
        </div>
        <div>
      ...
      {(onSetStatus || onDelete) && (
        <div className="flex flex-wrap items-center gap-2 border-t border-stone-200 pt-3">
          {onSetStatus && p.status !== "completed" && (
            <button type="button" onClick={() => onSetStatus(p.id, "completed")} className="inline-flex items-center gap-1 rounded-lg bg-green-50 px-2.5 py-1.5 text-xs font-bold text-green-700 ring-1 ring-green-600/15 hover:bg-green-100">
              <Check className="h-3.5 w-3.5" />{s.markCompleted}
            </button>
          )}
          {onSetStatus && p.status !== "failed" && (
            <button type="button" onClick={() => onSetStatus(p.id, "failed")} className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-2.5 py-1.5 text-xs font-bold text-red-700 ring-1 ring-red-600/15 hover:bg-red-100">
              <Ban className="h-3.5 w-3.5" />{s.markFailed}
            </button>
          )}
          {onSetStatus && p.status !== "pending" && (
            <button type="button" onClick={() => onSetStatus(p.id, "pending")} className="inline-flex items-center gap-1 rounded-lg bg-amber-50 px-2.5 py-1.5 text-xs font-bold text-amber-700 ring-1 ring-amber-600/15 hover:bg-amber-100">
              <Clock className="h-3.5 w-3.5" />{s.markPending}
            </button>
          )}
          {onDelete && (
            <button type="button" onClick={() => onDelete(p.id)} className="inline-flex items-center gap-1 rounded-lg bg-stone-100 px-2.5 py-1.5 text-xs font-bold text-stone-600 hover:bg-red-50 hover:text-red-700">
              <Trash2 className="h-3.5 w-3.5" />{s.deletePayment}
            </button>
          )}
        </div>
      )}
          ...
          return (
            <div key={p.id} className="p-4">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <span className="font-display text-lg font-black tabular-nums text-stone-800">{amountDisplay(p, lang)}</span>
              </div>
              <button type="button" onClick={() => toggle(p.id)} className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-xl border border-stone-200 py-2 text-sm font-bold text-stone-700 hover:bg-stone-50">
                <Eye className="h-4 w-4" />{s.view}
...
function RowGroup({
  p, open, s, lang, locale, onToggle, Detail, amountDisplay,
}: {
  p: Payment; open: boolean; s: PaymentStrings; lang: PayLang; locale: string;
  onToggle: () => void;
  Detail: ({ p }: { p: Payment }) => React.ReactNode;
  amountDisplay: (p: Payment, lang: PayLang) => string;
}) {
  return (
    <>
      <tr className="border-b border-stone-100 transition-colors hover:bg-stone-50">
        <td className="p-4">
          <button type="button" onClick={onToggle} aria-label="expand" className="grid h-7 w-7 place-items-center rounded-lg text-stone-400 hover:bg-stone-100">
            <ChevronDown className={`h-4 w-4 transition-transform ${open ? "rotate-180" : ""}`} />
          </button>
        </td>
        ...
        <td className="p-4 text-end font-display font-black tabular-nums text-stone-800">{amountDisplay(p, lang)}</td>
        <td className="p-4"><StatusBadge status={p.status} strings={s} /></td>
        <td className="p-4 text-end">
          <button type="button" onClick={onToggle} className="inline-flex items-center gap-1 rounded-lg bg-stone-100 px-2.5 py-1.5 text-xs font-bold text-stone-700 transition-colors hover:bg-stone-200">
