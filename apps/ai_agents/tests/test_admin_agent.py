...
async def test_admin_agent():
    ...
    login_resp = client.post(
        "/api/users/login", json={"email": "admin_test@example.com", "password": "securepass123"}
    )
    ...
    chat_resp = client.post(
        "/api/ai-agents/chat",
        headers=headers,
        json={"agent_type": "admin", "message": "لطفاً یک گزارش کلی از وضعیت پروژه ارائه بده."},
    )
    ...
    if prioritize_resp.status_code == 200:
        ...
        logger.error("❌ Prioritization failed")
        return
    ...
    if decision_resp.status_code == 200:
        ...
        logger.error("❌ Decision support failed")
        return
