...
async def test_ai_agents_module():
    ...
    login_resp = client.post(
        "/api/users/login", json={"email": "ai_test@example.com", "password": "securepass123"}
    )
    ...
    chat_resp = client.post(
        "/api/ai-agents/chat",
        headers=headers,
        json={"agent_type": "financial", "message": "لطفاً خلاصه‌ای از وضعیت کلی بازار ارائه بده."},
    )
    ...
    detail_resp = client.get(f"/api/ai-agents/conversations/{new_conv_id}", headers=headers)
    ...
    support_resp = client.post(
        "/api/ai-agents/chat",
        headers=headers,
        json={"agent_type": "support", "message": "چگونه می‌توانم رمز عبور خود را تغییر دهم؟"},
    )
