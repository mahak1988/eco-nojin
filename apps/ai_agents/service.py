...
class AIAgentService:
    ...
    async def create_conversation(
        self, user_id: int, agent_type: str, title: str | None = None
    ) -> Conversation:
        """ایجاد مکالمه جدید."""
    ...
    async def get_user_conversations(
        self, user_id: int, agent_type: str | None = None, limit: int = 50
    ):
        """دریافت مکالمات کاربر."""
        conversations = await self.conversation_repo.get_user_conversations(
            user_id=user_id, agent_type=agent_type, limit=limit
        )
    ...
    async def chat(self, user_id: int, request: ChatRequest) -> dict[str, Any]:
        ...
        if conversation_id:
            ...
            conv = await self.create_conversation(user_id=user_id, agent_type=request.agent_type)
        ...
        try:
            ...
            assistant_response = await agent.chat(user_message=request.message, context=context)
