...
class Account(Base):
    ...
    parent: Mapped[Account | None] = relationship("Account", remote_side=[id], backref="children")
    ...
    def balance(self, value: Decimal) -> None:
        self._balance = value if value is not None else Decimal("0.00")
...
class JournalEntry(Base):
    ...
    def total_debits(self) -> Decimal:
        return sum(
            (item.amount for item in self.items if item.entry_type == EntryType.DEBIT),
            Decimal("0.00"),
        )
    ...
    def total_credits(self) -> Decimal:
        return sum(
            (item.amount for item in self.items if item.entry_type == EntryType.CREDIT),
            Decimal("0.00"),
        )
...
class JournalItem(Base):
    ...
    entry_id: Mapped[str] = mapped_column(
        String(30), ForeignKey("journal_entries.id"), nullable=False, index=True
    )
...
class InvoiceItem(Base):
    ...
    invoice_id: Mapped[str] = mapped_column(
        String(30), ForeignKey("invoices.id"), nullable=False, index=True
    )
    description: Mapped[str] = mapped_column(String(500), nullable=False)
...
class BudgetAlert(Base):
    ...
    budget_id: Mapped[str] = mapped_column(
        String(30), ForeignKey("budgets.id"), nullable=False, index=True
    )
...
class FixedAsset(Base):
    ...
    depreciation_method: Mapped[str] = mapped_column(
        String(20), default="straight_line", nullable=False
    )
