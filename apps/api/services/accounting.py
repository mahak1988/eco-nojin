...
class JournalEntryService:
    ...
    async def create(self, data: JournalEntryCreate) -> JournalEntry:
        total_debits = sum(item.amount for item in data.items if item.entry_type == "debit")
        total_credits = sum(item.amount for item in data.items if item.entry_type == "credit")
