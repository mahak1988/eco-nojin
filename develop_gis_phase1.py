...
# -*- coding: utf-8 -*-
"""
🗺️ فاز ۱: توسعه هسته GIS پیشرفته
- نقشه Leaflet با لایه‌های متعدد
...
def install_dependencies():
    ...
    result = subprocess.run(cmd, cwd=str(WEB.parent), capture_output=True, text=True)
    
    if result.returncode == 0:
        print("   ✅ کتابخانه‌ها نصب شدند")
    else:
        print(f"   ⚠️  برخی کتابخانه‌ها نصب نشدند: {result.stderr}")
        print("   💡 نصب دستی: pnpm add leaflet react-leaflet @types/leaflet")


# ========== 2. کامپوننت نقشه پیشرفته ==========
def create_advanced_map():
    print("\n🗺️ ایجاد کامپوننت نقشه پیشرفته...")
...
import { 
  ...
  Maximize2, Download, Upload, Eye, EyeOff, Compass, Mountain
} from "lucide-react";

// Dynamic imports برای جلوگیری از SSR issues
...
const AttributionControl = dynamic(() => import("react-leaflet").then(mod => mod.AttributionControl), { ssr: false });
...
const OVERLAY_LAYERS = {
  ndvi: {
    ...
    url: "https://map1.vis.earthdata.nasa.gov/wmts-geo/VIIRS_SNPP_CorrectedReflectance_TrueColor/default/2024-01-01/GoogleMapsCompatible_Level9/{z}/{y}/{x}.jpg",
    opacity: 0.6,
    color: "#10b981"
  },
  drought: {
    name: "شاخص خشکسالی",
    url: "https://server.arcgisonline.com/ArcGIS/rest/services/Specialty/Soil_Survey_Map/MapServer/tile/{z}/{y}/{x}",
    opacity: 0.5,
    color: "#f59e0b"
  },
  elevation: {
...
export default function AdvancedGISMap() {
  ...
  return (
    <div className="relative h-full">
      ...
      {selectedRegion && (
        <div className="absolute bottom-4 right-4 z-[1000] bg-slate-900/95 backdrop-blur border border-slate-700 rounded-xl p-4 w-80">
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button className="py-2 px-3 bg-slate-800 hover:bg-slate-700 text-white rounded-lg">
      ...
      {/* Mouse Position */}
      <div className="absolute bottom-4 left-4 z-[1000] bg-slate-900/95 backdrop-blur border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-400">
        {mousePosition ? `${mousePosition.lat.toFixed(5)}, ${mousePosition.lng.toFixed(5)}` : "موقعیت ماوس"}
      </div>

      {/* Instructions */}
      {measureMode !== "none" && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[1000] bg-amber-500/95 backdrop-blur border border-amber-400 rounded-xl p-4 text-white pointer-events-none">
          <p className="font-bold mb-1">
            {measureMode === "distance" ? "📏 اندازه‌گیری مسافت" : "📐 اندازه‌گیری مساحت"}
          </p>
          <p className="text-sm">روی نقشه کلیک کنید</p>
          <p className="text-xs mt-2">نقاط: {measurePoints.length}</p>
        </div>
      )}
    </div>
  );
...
import { 
  ...
  Download, Mountain, Satellite, Compass, Globe
} from "lucide-react";
      ...
      <section className="container mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            {
              icon: Layers,
              title: "لایه‌های ماهواره‌ای",
              description: "دسترسی به تصاویر Sentinel-2، Landsat، MODIS و ماهواره‌های دیگر",
              color: "from-violet-500 to-purple-600"
            },
            {
              icon: Ruler,
              title: "ابزارهای اندازه‌گیری",
              description: "محاسبه دقیق مسافت، مساحت و محیط با کلیک روی نقشه",
              color: "from-blue-500 to-cyan-600"
            },
            {
              icon: TrendingUp,
              title: "شاخص‌های طیفی",
              description: "NDVI، EVI، SAVI، NDWI و سایر شاخص‌های پوشش گیاهی",
              color: "from-emerald-500 to-green-600"
            },
            {
              icon: Mountain,
              title: "تحلیل توپوگرافی",
              description: "شیب، جهت، TWI و مدل‌های ارتفاعی دیجیتال",
              color: "from-amber-500 to-orange-600"
            },
            {
              icon: Compass,
              title: "تحلیل حوضه آبریز",
              description: "delineation حوضه، جهت جریان، تجمع جریان",
              color: "from-sky-500 to-blue-600"
            },
            {
              icon: Globe,
              title: "خروجی‌های استاندارد",
              description: "GeoJSON، Shapefile، KML و گزارش‌های PDF",
...
def main():
    ...
    print("\n🎯 ویژگی‌های جدید:")
    print("   1. ✅ نقشه Leaflet با ۴ لایه پایه")
    print("      • OpenStreetMap")
    print("      • تصویر ماهواره‌ای (Esri)")
    print("      • توپوگرافی (OpenTopoMap)")
    print("      • حالت تاریک (CARTO)")
    print("")
    print("   2. ✅ ۴ لایه پوششی")
    print("      • NDVI (پوشش گیاهی)")
    print("      • شاخص خشکسالی")
    print("      • مدل ارتفاعی")
    print("      • شبکه آبراهه‌ها")
    print("")
    print("   3. ✅ ابزارهای اندازه‌گیری")
    print("      • اندازه‌گیری مسافت (کلیک چند نقطه)")
    print("      • اندازه‌گیری مساحت (رسم پلیگون)")
    print("      • نمایش مختصات real-time")
    print("")
    print("   4. ✅ پنل لایه‌ها")
    print("      • تغییر لایه پایه")
    print("      • روشن/خاموش کردن لایه‌های پوششی")
    print("      • نمایش اطلاعات منطقه")
    print("")
    print("   5. ✅ جستجوی مکان")
    print("   6. ✅ نمایش اطلاعات ۶ منطقه نمونه")
    print("   7. ✅ بخش ماهواره‌های در دسترس")
    ...
    print("      • لایه‌های مختلف را امتحان کنید")
    print("=" * 70)
