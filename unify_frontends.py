            ...
            if target_path.exists():
                ...
                if get_file_signature(path) == get_file_signature(target_path):
                    ...
                    stats["unique_files"].append(
                        {"path": str(rel_path), "status": "conflict", "note": "محتوای متفاوت"}
                    )
            else:
                stats["unique_files"].append(
                    {"path": str(rel_path), "status": "new", "note": "فایل جدید"}
                )
...
def phase_analyze():
    ...
    primary_stats = {
        ...
        "total_size_mb": sum(
            p.stat().st_size for p in PRIMARY.rglob("*") if p.is_file() and not should_ignore(p)
        )
        / 1_048_576,
    }
