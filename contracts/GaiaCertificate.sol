...
/**
 * @title GaiaCertificate
 * @dev Living NFT که با رشد فعالیت واقعی به‌روز می‌شود
 */
contract GaiaCertificate is ERC721, ERC721URIStorage, Ownable {
    ...
    event CertificateUpdated(
        uint256 indexed tokenId,
        uint256 newCarbonMilliKg,
        uint8 newGrowthStage
    );
    
    constructor(address _minerContract) 
        ERC721("Gaia Certificate", "GAIA") 
        Ownable(msg.sender) 
    {
    ...
    modifier onlyMinerOrOwner() {
        require(
            msg.sender == minerContract || msg.sender == owner(),
            "Not authorized"
        );
        _;
    }
