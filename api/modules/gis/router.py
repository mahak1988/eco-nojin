...
async def calculate_area(coordinates: list[list[float]] = Body(..., description="لیست مختصات [lon, lat]")):
    ...
    return {
        ...
        "perimeter_km": sum(((coordinates[i][0] - coordinates[(i+1)%n][0])**2 + (coordinates[i][1] - coordinates[(i+1)%n][1])**2)**0.5 for i in range(n)) / 1000,
        "unit": "km"
    }
