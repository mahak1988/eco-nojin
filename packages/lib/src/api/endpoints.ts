...
export const ENDPOINTS = {
  ...
  ECOCOIN: {
    ...
    STATS: "/ecocoin/stats",
    BALANCE: (address: string) => `/api/ecocoin/balance/${address}`,
    TRANSACTIONS: (address: string, limit: number = 50) => `/api/ecocoin/transactions/${address}?limit=${limit}`,
    STAKING_TIERS: "/api/ecocoin/staking/tiers",
    REWARDS: "/api/ecocoin/staking/tiers",
    MINING_RECENT: "/api/ecocoin/mining/recent",
    VERIFY: "/api/ecocoin/verify",
    WALLETS: {
      BY_ID: (id: number) => `/ecocoin/wallets/${id}`,
      ME: "/ecocoin/wallets/me",
    },
    TRANSFER: "/api/ecocoin/transfer",
    STAKE: "/api/ecocoin/staking/stake",
  },
  SOIL_WATER: {
    BASE: "/soil-water",
